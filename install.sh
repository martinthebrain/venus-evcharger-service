...
UPDATER_NICE_LEVEL="${VENUS_EVCHARGER_UPDATER_NICE_LEVEL:-15}"
UPDATER_WORK_ROOT_OVERRIDE="${VENUS_EVCHARGER_UPDATER_WORK_ROOT:-}"
BOOTSTRAP_WORK_ROOT="${UPDATER_WORK_ROOT_OVERRIDE:-${TMPDIR:-/tmp}/venus-evcharger-bootstrap-work}"
...
ensure_updater() {
	mkdir -p "$BOOTSTRAP_STATE_DIR"
	if ! mkdir -p "$BOOTSTRAP_WORK_ROOT"; then
		BOOTSTRAP_WORK_ROOT="${BOOTSTRAP_STATE_DIR}/work"
		mkdir -p "$BOOTSTRAP_WORK_ROOT"
	fi

	tmp_dir=$(mktemp -d "${BOOTSTRAP_WORK_ROOT}/bootstrap.XXXXXX")
	cleanup_tmp() {
		rm -rf "$tmp_dir"
	}
...
if [ -n "$UPDATER_WORK_ROOT_OVERRIDE" ]; then
	export VENUS_EVCHARGER_UPDATER_WORK_ROOT="$UPDATER_WORK_ROOT_OVERRIDE"
fi
set +e
run_resource_conscious_updater
updater_status=$?
