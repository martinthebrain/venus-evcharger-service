...
CARGO_CACHE=${CARGO_CACHE:-/tmp/venus-evcharger-dbus-adapter-cargo}
TARGET_CACHE=${TARGET_CACHE:-/tmp/venus-evcharger-dbus-adapter-arm-target}
TARGET=armv7-unknown-linux-gnueabihf
RUST_IMAGE=${RUST_IMAGE:-rust:1.88-bookworm@sha256:af306cfa71d987911a781c37b59d7d67d934f49684058f96cf72079c3626bfe0}
USE_HOST_TOOLCHAIN=${VENUS_EVCHARGER_DBUS_ADAPTER_USE_HOST_TOOLCHAIN:-0}
...
OUTPUT_NAME=venus-evcharger-dbus-adapter

if [ ! -f "$ROOT/Cargo.lock" ]; then
	echo "rust/dbus-adapter/Cargo.lock is required for reproducible builds" >&2
	exit 1
fi
...
0 | 1) ;;
*)
	echo "VENUS_EVCHARGER_DBUS_ADAPTER_USE_HOST_TOOLCHAIN must be 0 or 1" >&2
	exit 2
	;;
esac
...
if [ "$USE_HOST_TOOLCHAIN" = "1" ]; then
	...
	if ! command -v docker >/dev/null 2>&1; then
		echo "Reproducible ARM build requires Docker; set VENUS_EVCHARGER_DBUS_ADAPTER_USE_HOST_TOOLCHAIN=1 for an explicit host build" >&2
		exit 1
	fi
	docker run --rm \
