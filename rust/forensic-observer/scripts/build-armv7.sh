...
USE_HOST_TOOLCHAIN=${VENUS_EVCHARGER_OBSERVER_USE_HOST_TOOLCHAIN:-0}
...
0 | 1) ;;
*)
	echo "VENUS_EVCHARGER_OBSERVER_USE_HOST_TOOLCHAIN must be 0 or 1" >&2
	exit 2
	;;
esac
...
if [ "$USE_HOST_TOOLCHAIN" = "1" ]; then
	...
	if ! command -v docker >/dev/null 2>&1; then
		echo "Reproducible ARM build requires Docker; set VENUS_EVCHARGER_OBSERVER_USE_HOST_TOOLCHAIN=1 for an explicit host build" >&2
		exit 1
	fi
	docker run --rm \
