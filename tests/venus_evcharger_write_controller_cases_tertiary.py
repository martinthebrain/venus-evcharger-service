...
class TestDbusWriteControllerTertiary(DbusWriteControllerTestBase):
    ...
    def test_write_port_rejects_non_string_contract_results(self) -> None:
        ...
        port = write_port(service)

        with self.assertRaisesRegex(TypeError, "apply_phase_selection must return str"):
            port.apply_phase_selection("P1")
        with self.assertRaisesRegex(TypeError, "state.summary must return str"):
            port.state_summary()

    def test_auto_runtime_setting_write_publishes_and_persists_overrides(self) -> None:
        policy = AutoPolicy()
        policy.normal_profile.start_surplus_watts = 1500.0
        service = SimpleNamespace(
            virtual_mode=1,
            virtual_autostart=1,
            virtual_startstop=0,
    ...
    def test_auto_runtime_setting_write_updates_canonical_learn_charge_flag(self) -> None:
        policy = AutoPolicy()
        service = SimpleNamespace(
            virtual_mode=1,
            virtual_autostart=1,
    ...
    def test_mode_transition_and_publish_helpers_cover_remaining_branches(self) -> None:
        ...
        service._publish_dbus_field.assert_any_call("start_stop", 1, 100.0, force=True)
        service._publish_dbus_field.assert_any_call("enable", 1, 100.0, force=True)
