...
class TestDbusAdapterSchedulerContracts(unittest.TestCase):
    ...
    def test_next_due_sorts_by_time_then_declared_order(self) -> None:
        ...
        due = scheduler.next_due(now=10.0, circuit_state="ok", priority_allowed=lambda _priority: True)
        self.assertIsNotNone(due)
        assert due is not None
        self.assertEqual(due[0], "second")
        scheduler.next_read_at.update({"first": 5.0, "second": 5.0})
        tied = scheduler.next_due(now=10.0, circuit_state="ok", priority_allowed=lambda _priority: True)
    ...
    def test_success_force_due_and_error_backoff_are_exact(self) -> None:
        ...
        scheduler.record_success("grid", now=10.0, interval=-2.0)
        ...
        scheduler.record_success("grid", now=10.0, interval=4.0)
        ...
        floor.record_error("grid", now=10.0, interval=2.0)
    ...
    def test_discovery_lifecycle_and_priority_contract(self) -> None:
        discovery = DbusDiscoveryManager(interval_seconds=0.0)
        ...
        priorities: list[str] = []
        self.assertTrue(
            discovery.due(now=0.0, priority_allowed=lambda priority: priorities.append(priority) or True)
        )
        self.assertEqual(priorities, ["discovery"])
        discovery.record_success(now=10.0)
        self.assertEqual(discovery.last_success_at, 10.0)
        self.assertEqual(discovery.last_error, "")
        self.assertEqual(discovery.next_scan_at, 15.0)
        self.assertFalse(discovery.due(now=14.999, priority_allowed=lambda _priority: True))
        self.assertTrue(discovery.due(now=15.0, priority_allowed=lambda _priority: True))
        discovery.record_error(RuntimeError("offline"), now=20.0)
        self.assertEqual(discovery.last_error, "offline")
        self.assertEqual(discovery.next_scan_at, 25.0)

        slow = DbusDiscoveryManager(interval_seconds=900.0)
        slow.record_error(ValueError("bad"), now=20.0)
        self.assertEqual(slow.next_scan_at, 80.0)
