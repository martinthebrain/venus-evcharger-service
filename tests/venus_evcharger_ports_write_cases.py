# SPDX-License-Identifier: GPL-3.0-or-later
import unittest
from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from venus_evcharger.ports import WriteControllerPort
from venus_evcharger.ports.write_runtime import (
    _finite_service_float,
    _service_binary_flag,
    _set_finite_service_float,
    _set_service_binary_flag,
)


class TestWallboxPortsWrite(unittest.TestCase):
    def test_write_runtime_float_helper_preserves_zero_and_defaults(self) -> None:
        service = SimpleNamespace()

        self.assertEqual(_finite_service_float(service, "missing"), 0.0)
        self.assertEqual(_finite_service_float(service, "missing", default=3.5), 3.5)
        service.value = 0
        self.assertEqual(_finite_service_float(service, "value", default=3.5), 0.0)
        service.value = "bad"
        self.assertEqual(_finite_service_float(service, "value", default=3.5), 3.5)

        _set_finite_service_float(service, "target", 0)
        self.assertEqual(service.target, 0.0)
        _set_finite_service_float(service, "target", 3.5)
        self.assertEqual(service.target, 3.5)
        _set_finite_service_float(service, "target", "bad")
        self.assertEqual(service.target, 0.0)

    def test_write_runtime_binary_helper_preserves_default_and_storage_type(self) -> None:
        service = SimpleNamespace()

        self.assertEqual(_service_binary_flag(service, "missing"), 1)
        service.flag = "0"
        self.assertEqual(_service_binary_flag(service, "flag"), 0)
        service.flag = "bad"
        self.assertEqual(_service_binary_flag(service, "flag"), 1)
        service.flag = None
        self.assertEqual(_service_binary_flag(service, "flag"), 1)
        service.flag = 1
        self.assertEqual(_service_binary_flag(service, "flag"), 1)
        service.flag = 2
        self.assertEqual(_service_binary_flag(service, "flag"), 1)

        _set_service_binary_flag(service, "stored", "1")
        self.assertEqual(service.stored, 1)
        self.assertIs(type(service.stored), int)
        _set_service_binary_flag(service, "stored_bool", "1", as_bool=True)
        self.assertIs(service.stored_bool, True)
        _set_service_binary_flag(service, "stored_bool", "0", as_bool=True)
        self.assertIs(service.stored_bool, False)

    def test_base_ports_raise_for_unknown_attrs_and_missing_controller_bindings(self) -> None:
        service = SimpleNamespace(virtual_mode=0, virtual_autostart=1, virtual_startstop=0, virtual_enable=1, virtual_set_current=16.0, min_current=6.0, max_current=16.0, auto_start_condition_since=None, auto_stop_condition_since=None, manual_override_until=0.0, auto_manual_override_seconds=300.0, _auto_mode_cutover_pending=False, _ignore_min_offtime_once=False)
        write_port = WriteControllerPort(service)
        with self.assertRaises(AttributeError):
            getattr(write_port, "unknown_attr")
        with self.assertRaises(AttributeError):
            write_port.clear_auto_samples()

    def _service(self):
        auto = SimpleNamespace(
            clear_samples=MagicMock(),
            normalize_mode=MagicMock(return_value=1),
            mode_uses_auto_logic=MagicMock(return_value=True),
        )
        runtime = SimpleNamespace(
            queue_relay_command=MagicMock(),
            publish_local_pm_status=MagicMock(),
            worker_snapshot=MagicMock(return_value={"pm_status": None}),
            pending_relay_command=MagicMock(return_value=(None, None)),
            update_worker_snapshot=MagicMock(),
            phase_selection_requires_pause=MagicMock(return_value=False),
            apply_phase_selection=MagicMock(return_value="P1"),
        )
        state = SimpleNamespace(
            publish_field=MagicMock(),
            summary=MagicMock(return_value="state"),
            save_runtime_state=MagicMock(),
            save_runtime_overrides=MagicMock(),
            validate_runtime_config=MagicMock(),
        )
        return SimpleNamespace(
            virtual_mode=0, virtual_autostart=1, virtual_startstop=0, virtual_enable=1, virtual_set_current=16.0,
            min_current=6.0, max_current=16.0, auto_start_condition_since=None, auto_stop_condition_since=None,
            manual_override_until=0.0, auto_manual_override_seconds=300.0, _auto_mode_cutover_pending=False,
            _ignore_min_offtime_once=False, auto=auto, runtime=runtime, state=state,
            time_now=MagicMock(return_value=100.0),
        )

    def test_write_controller_port_forwards_state_and_methods(self) -> None:
        service = self._service()
        port = WriteControllerPort(service)
        port.virtual_mode = 2
        setattr(port, "virtual_autostart", "0")
        port.virtual_enable = 5
        setattr(port, "virtual_startstop", "bad")
        setattr(port, "auto_mode_cutover_pending", 1)
        setattr(port, "ignore_min_offtime_once", 0)
        self.assertEqual(service.virtual_mode, 1)
        self.assertEqual(service.virtual_autostart, 0)
        self.assertEqual(service.virtual_enable, 1)
        self.assertEqual(service.virtual_startstop, 0)
        self.assertTrue(port.auto_mode_cutover_pending)
        self.assertFalse(port.ignore_min_offtime_once)
        port.queue_relay_command(True, 100.0)
        service.runtime.queue_relay_command.assert_called_once_with(True, 100.0)

    def test_write_controller_port_requires_confirmed_off_before_skipping_cutover(self) -> None:
        service = self._service()
        service.runtime.worker_snapshot.return_value = {"pm_status": {"output": False}, "pm_confirmed": False}
        service.runtime.pending_relay_command.return_value = (False, 99.0)
        service._last_pm_status = {"output": False}
        service._last_pm_status_confirmed = False
        service._last_pm_status_at = 99.0
        port = WriteControllerPort(service)
        self.assertTrue(port.relay_may_be_on_for_cutover())
        service.runtime.worker_snapshot.return_value = {"pm_status": {"output": False}, "pm_confirmed": True, "pm_captured_at": 99.5}
        self.assertFalse(port.relay_may_be_on_for_cutover())

    def test_write_controller_port_covers_runtime_override_and_charger_error_paths(self) -> None:
        service = self._service()
        service.virtual_startstop = 1
        service.supported_phase_selections = ("P1",)
        service.runtime.worker_snapshot.return_value = {"pm_status": {"output": False}, "pm_confirmed": True}
        service.runtime.pending_relay_command.return_value = (None, None)
        service._last_pm_status = {"output": False}
        service._last_pm_status_confirmed = True
        service._last_pm_status_at = True
        port = WriteControllerPort(service)
        port.supported_phase_selections = ("P1_P2", "P1")
        port._software_update_run_requested_at = 42.5
        self.assertEqual(service.supported_phase_selections, ("P1_P2", "P1"))
        self.assertEqual(port._software_update_run_requested_at, 42.5)
        with patch.object(WriteControllerPort, "_relay_output_timestamp", return_value=None):
            self.assertIsNone(port._fresh_snapshot_output({"pm_status": {"output": False}, "pm_confirmed": True, "pm_captured_at": 99.5}, 100.0, 2.0))
        self.assertIsNone(port._fresh_last_output(100.0, 2.0))
        with self.assertRaisesRegex(RuntimeError, "set_enabled configured"):
            port.charger_set_enabled(True)
        with self.assertRaisesRegex(RuntimeError, "set_current configured"):
            port.charger_set_current(12.0)

    def test_write_controller_port_staleness_paths(self) -> None:
        service = self._service()
        service.runtime.pending_relay_command.return_value = (None, None)
        service._last_pm_status = {"output": False}
        service._last_pm_status_confirmed = True
        service._last_pm_status_at = 95.0
        service._worker_poll_interval_seconds = 1.0
        service.relay_sync_timeout_seconds = 2.0
        service.runtime.worker_snapshot.return_value = {"pm_status": {"output": False}, "pm_confirmed": True, "pm_captured_at": 95.0}
        port = WriteControllerPort(service)
        self.assertTrue(port.relay_may_be_on_for_cutover())
        service.runtime.worker_snapshot.return_value = {"pm_status": None, "pm_confirmed": False}
        service._last_pm_status = None
        service._last_pm_status_confirmed = False
        service._last_pm_status_at = None
        self.assertTrue(port.relay_may_be_on_for_cutover())

    def test_write_controller_port_covers_fresh_last_output_and_validation_fallback(self) -> None:
        service = self._service()
        service.runtime.pending_relay_command.return_value = (None, None)
        service.runtime.worker_snapshot.return_value = {"pm_status": None, "pm_confirmed": False}
        service._last_confirmed_pm_status = {"output": True}
        service._last_confirmed_pm_status_at = 99.5
        service._worker_poll_interval_seconds = 1.0
        service.relay_sync_timeout_seconds = 2.0
        port = WriteControllerPort(service)

        self.assertTrue(port._fresh_last_output(100.0, 2.0))
        self.assertTrue(port._fresh_confirmed_relay_output({"pm_status": None, "pm_confirmed": False}) is True)
        port.validate_runtime_config()
        service.state.validate_runtime_config.assert_called_once_with()
