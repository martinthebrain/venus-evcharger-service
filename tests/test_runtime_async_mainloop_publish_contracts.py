...
class RuntimeAsyncMainloopPublishContractTests(unittest.TestCase):
    ...
    def test_update_index_direct_wraps_only_after_255_and_records_state(self) -> None:
        ...
        controller.publish_queue._bump_update_index_direct(service, 10.0)
        ...
        controller.publish_queue._bump_update_index_direct(service, 11.0)
        ...
        controller.publish_queue._bump_update_index_direct(service, 12.0)
    ...
    def test_drain_returns_every_value_and_resets_all_queue_metadata(self) -> None:
        ...
        drained = controller.publish_queue._drain_dbus_publish_queue(service)
    ...
    def test_drain_reports_each_queue_contract_name(self) -> None:
        ...
        with self.assertRaisesRegex(TypeError, "_dbus_publish_pending must be OrderedDict"):
            controller.publish_queue._drain_dbus_publish_queue(service)
    ...
    def test_queue_lag_is_exact_clamped_and_unchanged_without_timestamp(self) -> None:
        ...
        controller.publish_queue._remember_dbus_publish_queue_lag(service, 100.0, None)
        self.assertEqual(service._last_dbus_publish_queue_lag_seconds, 7.0)
        controller.publish_queue._remember_dbus_publish_queue_lag(service, 100.0, 98.5)
        self.assertEqual(service._last_dbus_publish_queue_lag_seconds, 1.5)
        controller.publish_queue._remember_dbus_publish_queue_lag(service, 100.0, 101.0)
    ...
    def test_field_conversion_keeps_only_known_consistent_paths(self) -> None:
        ...
        self.assertEqual(
            controller.publish_queue._path_values_from_fields(fields),
            [
                ("/Ac/Power", (1200.0, 10.0, 8.0)),
                ("/Session/Time", (30, 11.0, 9.0)),
            ],
        )
        self.assertEqual(controller.publish_queue._field_paths(fields), ["/Ac/Power", "/Session/Time"])
    ...
    def test_failure_reporting_marks_dbus_and_logs_exact_paths(self) -> None:
        ...
        service = SimpleNamespace(runtime=SimpleNamespace(mark_failure=mark_failure))

        with patch("venus_evcharger.runtime.async_mainloop_publish.logging.warning") as warning:
            controller.publish_queue._report_dbus_publish_failures(service, [])
            warning.assert_not_called()
            controller.publish_queue._report_dbus_publish_failures(service, ["/A", "/B"])
    ...
    def test_update_index_flushes_exact_count_and_stops_after_failure(self) -> None:
        service, controller = _controller()
        controller.publish_queue._bump_update_index_best_effort = MagicMock(side_effect=(True, False, True))

        controller.publish_queue._flush_update_index_bumps(service, 44.0, 0)
        controller.publish_queue._bump_update_index_best_effort.assert_not_called()
        controller.publish_queue._flush_update_index_bumps(service, 44.0, 3)

        self.assertEqual(
            controller.publish_queue._bump_update_index_best_effort.call_args_list,
            [call(service, 44.0), call(service, 44.0)],
        )

    def test_best_effort_bump_reports_exact_success_and_failure(self) -> None:
        service, controller = _controller()
        controller.publish_queue._bump_update_index_direct = MagicMock()
        self.assertTrue(controller.publish_queue._bump_update_index_best_effort(service, 55.0))
        controller.publish_queue._bump_update_index_direct.assert_called_once_with(service, 55.0)

        controller.publish_queue._bump_update_index_direct.side_effect = RuntimeError("down")
        with patch("venus_evcharger.runtime.async_mainloop_publish.logging.warning") as warning:
            self.assertFalse(controller.publish_queue._bump_update_index_best_effort(service, 56.0))
    ...
    def test_flush_duration_uses_strict_budget_and_invalid_default(self) -> None:
        ...
        with (
            ...
            controller.publish_queue._record_publish_flush_duration(service, 10.0)
        ...
        with (
            ...
            controller.publish_queue._record_publish_flush_duration(service, 20.0)
        warning.assert_not_called()
        ...
        with (
            ...
            controller.publish_queue._record_publish_flush_duration(service, 30.0)
    ...
    def test_flush_orchestrates_all_steps_with_exact_timing_values(self) -> None:
        ...
        controller.publish_queue._drain_dbus_publish_queue = MagicMock(return_value=(values, fields, 2, 8.0))
        controller.publish_queue._remember_dbus_publish_queue_lag = MagicMock()
        controller.publish_queue._apply_dbus_publish_values = MagicMock(return_value=["/A"])
        controller.publish_queue._apply_dbus_publish_fields = MagicMock(return_value=["/Ac/Power"])
        controller.publish_queue._report_dbus_publish_failures = MagicMock()
        controller.publish_queue._flush_update_index_bumps = MagicMock()
        controller.publish_queue._record_publish_flush_duration = MagicMock()
        controller.mainloop_watchdog.flush_companion_publish = MagicMock()
        ...
        with (
            ...
            self.assertTrue(controller.publish_queue.flush())

        controller.async_state.assert_mainloop_thread.assert_called_once_with("main DBus publish flush")
        controller.publish_queue._remember_dbus_publish_queue_lag.assert_called_once_with(service, 60.0, 8.0)
        controller.publish_queue._apply_dbus_publish_values.assert_called_once_with(service, values)
        controller.publish_queue._apply_dbus_publish_fields.assert_called_once_with(service, fields)
        controller.publish_queue._report_dbus_publish_failures.assert_called_once_with(service, ["/A", "/Ac/Power"])
        controller.publish_queue._flush_update_index_bumps.assert_called_once_with(service, 60.0, 2)
        controller.publish_queue._record_publish_flush_duration.assert_called_once_with(service, 50.0)
        controller.mainloop_watchdog.flush_companion_publish.assert_called_once_with()
