...
class _TestShellyIoControllerTertiaryPart2:
    def test_shelly_error_classification_covers_common_network_failures(self):
        ...
        self.assertEqual(controller.transport.classify_error(requests.exceptions.ConnectTimeout()), "connect-timeout")
        self.assertEqual(controller.transport.classify_error(requests.exceptions.ReadTimeout()), "read-timeout")
        self.assertEqual(controller.transport.classify_error(requests.exceptions.Timeout()), "timeout")
        self.assertEqual(
            controller.transport.classify_error(requests.exceptions.ConnectionError("No route to host")),
            "no-route",
        )
        self.assertEqual(
            controller.transport.classify_error(requests.exceptions.ConnectionError("Connection refused")),
            "connection-refused",
        )
        self.assertEqual(
            controller.transport.classify_error(requests.exceptions.ConnectionError("reset by peer")),
            "connection-error",
        )
        self.assertEqual(controller.transport.classify_error(json.JSONDecodeError("bad", "{}", 0)), "bad-json")
        response = SimpleNamespace(status_code=401)
        self.assertEqual(controller.transport.classify_error(requests.exceptions.HTTPError(response=response)), "auth-error")
        response = SimpleNamespace(status_code=403)
        self.assertEqual(controller.transport.classify_error(requests.exceptions.HTTPError(response=response)), "auth-error")
        response = SimpleNamespace(status_code=500)
        self.assertEqual(controller.transport.classify_error(requests.exceptions.HTTPError(response=response)), "http-error")
        self.assertEqual(controller.transport.classify_error(requests.exceptions.HTTPError()), "http-error")
        self.assertEqual(controller.transport.classify_error(ValueError("program error")), "error")

    def test_shelly_retry_state_helpers_cover_fallbacks_and_invalid_values(self):
        controller = ShellyIoController(SimpleNamespace())
        self.assertEqual(
            controller.transport._shelly_retry_after_value(SimpleNamespace(_shelly_retry_after=42.5)),
            42.5,
        )
        self.assertEqual(
            controller.transport._shelly_retry_after_value(SimpleNamespace(_source_retry_after={"shelly": 77.0})),
            77.0,
        )
        self.assertEqual(
            controller.transport._shelly_retry_after_value(SimpleNamespace(_source_retry_after={})),
            0.0,
        )
        self.assertEqual(controller.transport._shelly_retry_after_value(SimpleNamespace(_shelly_retry_after=True)), 0.0)
        self.assertEqual(
            controller.transport._shelly_retry_after_value(
        ...
        self.assertFalse(controller.transport.retry_active(100.0))
        source_retry_ready = MagicMock(return_value=False)
        retry_controller = ShellyIoController(SimpleNamespace())
        retry_controller.service.runtime.source_retry_ready = source_retry_ready
        self.assertTrue(retry_controller.transport.retry_active(12.5))
        ...
        self.assertTrue(
            ...
            .transport.retry_active(100.0)
        )
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("auth-error", 1), 60.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("bad-json", 0), 1.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("bad-json", 20), 15.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("unknown", 1), 1.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("connect-timeout", 1), 2.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("connection-refused", 1), 10.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("http-error", 1), 5.0)
        self.assertEqual(controller.transport._shelly_retry_delay_seconds("timeout", 6), 16.0)

    def test_common_network_read_failure_is_tolerated_while_success_is_fresh(self):
        service = SimpleNamespace(
            auto_shelly_soft_fail_seconds=10.0,
            _shelly_last_ok_at=95.0,
            _last_confirmed_pm_status_at=None,
        )
        transport = ShellyIoController(service).transport
        timeout = requests.exceptions.ConnectTimeout("slow")

        self.assertTrue(transport.read_failure_is_tolerated(timeout, 100.0))
        service._shelly_last_ok_at = 89.9
        self.assertFalse(transport.read_failure_is_tolerated(timeout, 100.0))
        service._last_confirmed_pm_status_at = 99.0
        self.assertTrue(transport.read_failure_is_tolerated(timeout, 100.0))
        self.assertFalse(transport.read_failure_is_tolerated(json.JSONDecodeError("bad", "{}", 0), 100.0))
        service.auto_shelly_soft_fail_seconds = None
        self.assertFalse(transport.read_failure_is_tolerated(timeout, 100.0))
        service.auto_shelly_soft_fail_seconds = 0.0
        service._shelly_last_ok_at = 100.0
        self.assertFalse(transport.read_failure_is_tolerated(timeout, 100.0))
        service.auto_shelly_soft_fail_seconds = 1.0
        service._shelly_last_ok_at = 99.0
        self.assertTrue(transport.read_failure_is_tolerated(timeout, 100.0))
        service._shelly_last_ok_at = 100.5
        self.assertTrue(transport.read_failure_is_tolerated(timeout, 100.0))

    def test_tolerated_network_read_gap_keeps_health_clear_and_snapshot_unconfirmed(self):
        ...
        controller.worker._fetch_pm_status = MagicMock(side_effect=requests.exceptions.ConnectTimeout("slow"))
        ...
        self.assertFalse(service.runtime.update_worker_snapshot.call_args_list[-1].kwargs["pm_confirmed"])

    def test_network_read_grace_is_evaluated_when_timeout_finishes(self):
        service = SimpleNamespace(
            time_now=MagicMock(side_effect=[100.0, 102.0]),
            virtual_mode=1,
            auto_shelly_soft_fail_seconds=10.0,
            _shelly_last_ok_at=91.0,
        ...
        self.assertEqual(service._shelly_last_error_at, 102.0)
        self.assertEqual(service.runtime.update_worker_snapshot.call_args_list[-1].kwargs["captured_at"], 102.0)
    ...
    def test_remember_shelly_failure_passes_complete_failure_contract(self):
        ...
        with (
            patch.object(controller.transport, "_record_shelly_failure_state") as record_state,
            patch.object(controller.transport, "_record_shelly_retry_after") as record_retry,
            patch.object(controller.transport, "_reset_shelly_worker_session") as reset_session,
        ):
            controller.transport.remember_failure("read-timeout", "pm-status", error, 10.0)
        ...
        record_retry.assert_called_once_with(service, 10.0, 4.0)
        reset_session.assert_called_once_with()

        zero_counter_service = SimpleNamespace(_shelly_consecutive_errors=0)
        zero_counter_controller = ShellyIoController(zero_counter_service)
        with (
            patch.object(zero_counter_controller.transport, "_record_shelly_failure_state") as record_state,
            patch.object(zero_counter_controller.transport, "_record_shelly_retry_after"),
            patch.object(zero_counter_controller.transport, "_reset_shelly_worker_session"),
        ):
            zero_counter_controller.transport.remember_failure("timeout", "poll", error, 20.0)
    ...
    def test_record_shelly_failure_state_sets_diagnostics_and_offline_start_once(self):
        ...
        controller.transport._record_shelly_failure_state(
        ...
        controller.transport._record_shelly_offline_since(offline_service, 110.0)
    ...
    def test_remember_shelly_failure_covers_invalid_counter_and_source_retry_dict(self):
        ...
        with patch("venus_evcharger.backend.shelly_io_worker_transport.requests.Session", return_value=MagicMock()):
            controller.transport.remember_failure(
                "auth-error",
                "read",
    ...
    def test_reset_shelly_worker_session_covers_invalid_reset_count(self):
        ...
        with patch("venus_evcharger.backend.shelly_io_worker_transport.requests.Session", side_effect=[MagicMock(), MagicMock()]):
            controller.transport._reset_shelly_worker_session()
    ...
    def test_io_worker_once_classifies_no_route_sets_backoff_and_resets_session(self):
        ...
        controller = ShellyIoController(service)
        controller.worker.worker_apply_pending_relay_command = MagicMock()
        controller.worker._fetch_pm_status = MagicMock(
            side_effect=requests.exceptions.ConnectionError("No route to host")
        )

        with patch("venus_evcharger.backend.shelly_io_worker_transport.requests.Session", return_value=new_session):
            controller.io_worker_once()

        service.runtime.delay_source_retry.assert_called_once_with("shelly", 100.0, 30.0)
        ...
        self.assertEqual(service._shelly_consecutive_errors, 1)
        self.assertEqual(service._shelly_session_reset_count, 1)
        self.assertIsNone(service.runtime.warning_throttled.call_args.kwargs.get("exc_info"))
    ...
    def test_shelly_transport_failure_resets_split_backend_sessions(self):
        ...
        controller = ShellyIoController(service)
        controller.worker.worker_apply_pending_relay_command = MagicMock()
        controller.worker._fetch_pm_status = MagicMock(side_effect=requests.exceptions.ConnectTimeout("timeout"))
    ...
    def test_io_worker_once_classifies_bad_json_without_transport_session_reset(self):
        ...
        controller = ShellyIoController(service)
        controller.worker.worker_apply_pending_relay_command = MagicMock()
        controller.worker._fetch_pm_status = MagicMock(side_effect=json.JSONDecodeError("broken", "{}", 0))

        controller.io_worker_once()

        service.runtime.delay_source_retry.assert_called_once_with("shelly", 100.0, 1.0)
    ...
    def test_worker_skips_shelly_read_and_pending_relay_while_retry_backoff_is_active(self):
        ...
        controller.worker._fetch_pm_status.assert_not_called()
        ...
        relay_service.runtime.source_retry_ready.side_effect = None
        relay_service.runtime.source_retry_ready.return_value = False
    ...
    def test_shelly_success_clears_retry_state(self):
        ...
        controller = ShellyIoController(service)
        service.runtime.source_retry_ready.side_effect = None
        service.runtime.source_retry_ready.return_value = True
        controller.worker.worker_apply_pending_relay_command = MagicMock()
        controller.worker._fetch_pm_status = MagicMock(return_value={"output": True})
    ...
    def test_reset_shelly_worker_session_resets_empty_worker_and_counts_from_zero(self):
        service = SimpleNamespace(
    ...
    def test_reset_shelly_worker_session_increments_existing_counter_and_deduplicates_backends(self):
        ...
        self.assertEqual(
            ShellyWorkerTransport._shelly_transport_backends(complete_service),
            (meter_backend, switch_backend, charger_backend),
        )
        self.assertEqual(
            ShellyWorkerTransport._shelly_transport_backends(
        ...
        self.assertEqual(controller.transport._shelly_transport_backends(service), (meter_backend, switch_backend))
        with patch(
            "venus_evcharger.backend.shelly_io_worker_transport.requests.Session",
    ...
    def test_io_worker_loop_logs_cycle_failure_and_continues(self):
        ...
        service = SimpleNamespace(
            ...
            time_now=MagicMock(side_effect=[100.0, 100.2, 101.0, 101.1]),
            _worker_poll_interval_seconds=1.0,
        )

        controller = ShellyIoController(service)
        controller.worker.io_worker_once = MagicMock(side_effect=[error, None])

        controller.io_worker_loop()

        service.runtime.ensure_worker_state.assert_called_once_with()
        self.assertEqual(controller.worker.io_worker_once.call_count, 2)
        service.runtime.warning_throttled.assert_called_once()
        args = service.runtime.warning_throttled.call_args[0]
        ...
        self.assertIs(service.runtime.warning_throttled.call_args.kwargs["exc_info"], error)
    ...
    def test_worker_loop_wait_seconds_respects_minimum_and_elapsed_time(self):
        service = SimpleNamespace(
            time_now=MagicMock(return_value=103.0),
            _worker_poll_interval_seconds=5.0,
        )
        controller = ShellyIoController(service)

        self.assertEqual(controller.worker._worker_loop_wait_seconds(service, 100.0), 2.0)
        service.time_now.return_value = 110.0
        self.assertEqual(controller.worker._worker_loop_wait_seconds(service, 100.0), 0.05)
