...
class TestShellyWallboxInvariants(unittest.TestCase):
    ...
    def test_auto_controller_components_share_one_explicit_service_boundary(self):
        controller, service = self._make_controller()

        self.assertIs(controller.context.service, service)
        self.assertIs(controller.learning.service, service)
        self.assertIs(controller.samples.service, service)
        self.assertIs(controller.metrics.service, service)
        self.assertEqual(controller.get_available_surplus_watts(2000, -1000), 1000.0)
    ...
    def _run_auto_sequence(self, samples):
        ...
        service.auto_policy.stop_surplus_delay_seconds = 20.0
        service.auto_min_runtime_seconds = 60.0
        service.auto_min_offtime_seconds = 60.0
        service.auto_policy.start_max_grid_import_watts = 50.0
        relay_on = False
        transitions = 0
        now = 0.0
        ...
        for pv_power, battery_soc, grid_power, step_seconds in samples:
            ...
            service.time_now = lambda current=now: current
            with patch("venus_evcharger.auto.logic_learning.time.time", return_value=now):
    ...
    def test_threshold_invariants_hold_across_generated_policies(self):
        ...
        for case_index in range(100):
            ...
            with self.subTest(case=case_index, profile="normal"):
                ...
                start_threshold, stop_threshold, _ = controller.learning._surplus_thresholds_for_soc(
                ...
                self.assertGreaterEqual(controller.learning._learned_charge_power_scale(1000.0), 0.0)
        ...
        for case_index in range(80):
            ...
            for profile_name, soc_value in (
                ...
                with self.subTest(case=case_index, profile=profile_name):
                    ...
                    stale_thresholds = stale_controller.learning._surplus_thresholds_for_soc(soc_value)
                    static_thresholds = static_controller.learning._surplus_thresholds_for_soc(soc_value)
                    self.assertEqual(stale_thresholds, static_thresholds)
                    self.assertEqual(stale_controller.learning._learned_charge_power_scale(1000.0), 1.0)
                    self.assertIsNone(stale_controller.learning._active_learned_charge_power(1000.0))
