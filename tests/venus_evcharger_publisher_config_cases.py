...
class TestDbusPublishControllerConfig(DbusPublishControllerTestCase):
    ...
    def test_config_values_publish_live_connected_display(self) -> None:
        ...
        self.assertEqual(controller.config._config_values(1, now=100.0)["connected"], 0)
        service._shelly_state = ""
        self.assertEqual(controller.config._config_values(1, now=109.0)["connected"], 1)
        self.assertEqual(controller.config._config_values(1, now=120.1)["connected"], 0)
    ...
    def test_config_helpers_cover_fault_and_contactor_count_fallbacks(self) -> None:
        ...
        self.assertEqual(controller.runtime_view.fault_reason(service), "contactor-lockout-open")
    ...
    def test_backend_helpers_prefer_config_over_conflicting_legacy_attributes(self) -> None:
        ...
        self.assertEqual(controller.runtime_view.backend_type_value(service, "switch_backend_type", "switch"), "switch_group")
        self.assertEqual(controller.runtime_view.backend_type_value(service, "charger_backend_type", "na"), "goe_charger")
    ...
    def test_config_helper_contracts_cover_backend_phase_contactor_and_schedule_edges(self) -> None:
        ...
        self.assertTrue(controller.config._service_configured_for_connected(service))
        self.assertFalse(controller.config._service_configured_for_connected(SimpleNamespace(topology_configured=False)))
        self.assertFalse(controller.config._service_configured_for_connected(SimpleNamespace(host_configured=False)))
        self.assertTrue(controller.config._service_configured_for_connected(SimpleNamespace(host_configured=True)))
        self.assertTrue(controller.config._service_configured_for_connected(SimpleNamespace()))
        self.assertTrue(controller.config._connected_state_is_live("online"))
        self.assertTrue(controller.config._connected_state_is_live("degraded"))
        self.assertFalse(controller.config._connected_state_is_live("offline"))
        self.assertFalse(controller.config._connected_state_is_live("unknown"))
        self.assertEqual(controller.config._explicit_connected_state_display("degraded"), 1)
        self.assertIsNone(controller.config._explicit_connected_state_display("unknown"))
        self.assertEqual(controller.config._backend_reachable_display(service, 100.0), 1)
        service._shelly_state = "offline"
        self.assertEqual(controller.config._backend_reachable_display(service, 100.0), 0)
        service._shelly_state = ""
        self.assertEqual(controller.config._implicit_connected_display(service, 100.0), 1)
        self.assertTrue(controller.config._fresh_backend_transport_problem(service, 100.0))
        for timestamp_name in (
            ...
            self.assertTrue(controller.config._fresh_backend_readback_present(timestamp_service, 100.0))
        self.assertFalse(controller.config._fresh_backend_readback_present(SimpleNamespace(auto_shelly_soft_fail_seconds=4.0), 100.0))
        self.assertTrue(controller.config._connected_timestamp_fresh(SimpleNamespace(auto_shelly_soft_fail_seconds=4.0, marker=92.0), "marker", 100.0))
        self.assertFalse(controller.config._connected_timestamp_fresh(SimpleNamespace(auto_shelly_soft_fail_seconds=4.0, marker=91.9), "marker", 100.0))
        self.assertEqual(controller.config._implicit_connected_display(SimpleNamespace(_last_pm_status_at=None), 100.0), 1)
        self.assertEqual(
            controller.config._implicit_connected_display(
        ...
        self.assertEqual(controller.config._connected_stale_after_seconds(service), 8.0)
        self.assertEqual(controller.config._connected_stale_after_seconds(SimpleNamespace()), 20.0)
        service.auto_shelly_soft_fail_seconds = 0.2
        self.assertEqual(controller.config._connected_stale_after_seconds(service), 1.0)
        service.auto_shelly_soft_fail_seconds = 4.0

        self.assertEqual(controller.runtime_view.backend_mode_value(service), "combined")
        self.assertEqual(controller.runtime_view.backend_type_value(service, "meter_backend_type", "meter"), "meter")
        self.assertEqual(controller.runtime_view.backend_type_value(service, "unknown_backend_type", "fallback"), "fallback")
        self.assertEqual(controller.runtime_view.backend_type_value(SimpleNamespace(), "unknown_backend_type"), "")
        self.assertEqual(
            controller.runtime_view.backend_type_value(
                SimpleNamespace(custom_backend=" custom "),
                "custom_backend",
                "fallback",
            ),
            "custom",
        ...
        for raw_value, expected in (
            ...
            with self.subTest(raw_value=raw_value):
                self.assertEqual(
                    controller.runtime_view.backend_type_value(unknown_service, "unknown_backend_type", "fallback"),
                    expected,
                )
        self.assertEqual(controller.runtime_view.charger_current_target_value(service), -1.0)
        service._charger_target_current_amps = 13.5
        self.assertEqual(controller.runtime_view.charger_current_target_value(service), 13.5)
        self.assertEqual(controller.runtime_view.auto_phase_metric_text(service, "phase_target"), "P1_P2")
        self.assertEqual(controller.runtime_view.auto_phase_metric_float(service, "phase_threshold_watts"), 1750.5)
        service._last_health_reason = "contactor-lockout-open"
        self.assertEqual(controller.runtime_view.fault_reason(service), "contactor-lockout-open")
        self.assertEqual(controller.runtime_view.fault_active(service), 1)

        snapshot = controller.runtime_view.scheduled_snapshot(service, 1719817200.0)
        ...
        non_default_snapshot = controller.runtime_view.scheduled_snapshot(service, 1782932400.0)
        ...
        self.assertIsNone(controller.runtime_view.scheduled_snapshot(service, 1719817200.0))
        service.virtual_mode = 2

        self.assertEqual(controller.runtime_view.recovery_active(service), 1)
        service._last_auto_state = "idle"
        self.assertEqual(controller.runtime_view.recovery_active(service), 0)
        service._last_auto_state = "recovery"
        self.assertEqual(controller.runtime_view.observed_phase_value(service), "P1_P2")
        service._last_confirmed_pm_status = {}
        self.assertEqual(controller.runtime_view.observed_phase_value(service), "P1")
        service._last_health_reason = "phase-switch-mismatch"
        self.assertEqual(controller.runtime_view.phase_switch_mismatch_active(service), 1)
        self.assertEqual(controller.runtime_view.phase_switch_lockout_active(service, 100.0), 1)
        self.assertEqual(controller.runtime_view.phase_switch_lockout_target(service, 100.0), "P1_P2")
        self.assertEqual(controller.runtime_view.phase_switch_lockout_reason(service, 100.0), "mismatch-threshold")
        self.assertEqual(
            controller.runtime_view.phase_switch_lockout_active(
        ...
        self.assertEqual(controller.runtime_view.phase_supported_configured(service), "P1,P1_P2")
        self.assertEqual(controller.runtime_view.phase_supported_effective(service, 100.0), "P1")
        self.assertEqual(controller.runtime_view.phase_degraded_active(service, 100.0), 1)
        service._phase_switch_lockout_until = 90.0
        self.assertEqual(controller.runtime_view.phase_switch_lockout_active(service, 100.0), 0)
        self.assertEqual(controller.runtime_view.phase_supported_effective(service, 100.0), "P1,P1_P2")

        service._last_confirmed_pm_status = {"output": True}
        self.assertEqual(controller.runtime_view.switch_feedback_closed(service), 0)
        self.assertEqual(controller.runtime_view.switch_interlock_ok(service), -1)
        self.assertEqual(controller.runtime_view.switch_feedback_mismatch(service), 1)
        self.assertEqual(
            controller.runtime_view.switch_feedback_mismatch(SimpleNamespace(_last_switch_feedback_closed=True)),
            1,
        )
        service._last_switch_feedback_closed = True
        service._last_confirmed_pm_status = {"output": True}
        self.assertEqual(controller.runtime_view.switch_feedback_mismatch(service), 0)
        ...
        self.assertEqual(controller.runtime_view.switch_feedback_mismatch(service), 1)
        self.assertEqual(controller.runtime_view.contactor_suspected_open(service), 0)
        service._last_health_reason = "contactor-suspected-open"
        self.assertEqual(controller.runtime_view.contactor_suspected_open(service), 1)
        self.assertEqual(controller.runtime_view.contactor_suspected_welded(service), 0)
        service._last_health_reason = "contactor-suspected-welded"
        self.assertEqual(controller.runtime_view.contactor_suspected_welded(service), 1)
        self.assertEqual(controller.runtime_view.contactor_lockout_reason(service), "contactor-suspected-open")
        self.assertEqual(controller.runtime_view.contactor_lockout_active(service), 1)
        self.assertEqual(controller.runtime_view.contactor_lockout_source(service), "count-threshold")
        self.assertEqual(controller.runtime_view.contactor_fault_count(service), 7)
        service._contactor_lockout_reason = ""
        self.assertEqual(controller.runtime_view.contactor_fault_count(service), 5)
    ...
    def test_config_helper_contracts_cover_missing_runtime_defaults(self) -> None:
        ...
        boundary_snapshot = controller.runtime_view.scheduled_snapshot(SimpleNamespace(virtual_mode=2), 1719774000.0)
        ...
        self.assertIsNone(controller.runtime_view.scheduled_snapshot(default_service, 1719817200.0))
        self.assertEqual(controller.runtime_view.recovery_active(default_service), 0)
        self.assertEqual(controller.runtime_view.phase_switch_mismatch_active(default_service), 0)
        self.assertEqual(controller.runtime_view.phase_switch_lockout_active(default_service, 100.0), 0)
        self.assertEqual(controller.runtime_view.phase_switch_lockout_target(default_service, 100.0), "")
        self.assertEqual(controller.runtime_view.phase_switch_lockout_reason(default_service, 100.0), "")
        self.assertEqual(controller.runtime_view.phase_supported_configured(default_service), "P1")
        self.assertEqual(controller.runtime_view.phase_supported_effective(default_service, 100.0), "P1")
        self.assertEqual(controller.runtime_view.phase_degraded_active(default_service, 100.0), 0)
        self.assertEqual(controller.runtime_view.switch_feedback_closed(default_service), -1)
        self.assertEqual(controller.runtime_view.switch_interlock_ok(default_service), -1)
        self.assertEqual(controller.runtime_view.switch_feedback_mismatch(default_service), 0)
        self.assertEqual(controller.runtime_view.contactor_suspected_open(default_service), 0)
        self.assertEqual(controller.runtime_view.contactor_suspected_welded(default_service), 0)
        self.assertEqual(controller.runtime_view.contactor_lockout_reason(default_service), "")
        self.assertEqual(controller.runtime_view.contactor_lockout_active(default_service), 0)
        self.assertEqual(controller.runtime_view.contactor_lockout_source(default_service), "")
        self.assertEqual(controller.runtime_view.contactor_fault_count(default_service), 0)
        ...
        self.assertEqual(controller.runtime_view.phase_switch_lockout_active(expired_lockout, 100.0), 0)
        self.assertEqual(controller.runtime_view.phase_switch_lockout_target(expired_lockout, 100.0), "")
        self.assertEqual(controller.runtime_view.phase_switch_lockout_reason(expired_lockout, 100.0), "")
        self.assertEqual(controller.runtime_view.phase_supported_effective(expired_lockout, 100.0), "P1,P1_P2")
    ...
    def test_scheduled_snapshot_passes_explicit_schedule_boundary_contract(self) -> None:
        ...
        with (
            patch(
                "venus_evcharger.publish.dbus_runtime_view.local_datetime_from_timestamp",
                return_value=scheduled_when,
            ) as local_datetime,
            patch(
                "venus_evcharger.publish.dbus_runtime_view.scheduled_mode_snapshot",
                return_value=expected_snapshot,
            ) as scheduled_snapshot,
        ):
            snapshot = controller.runtime_view.scheduled_snapshot(service, 1782932400.0)
    ...
    def test_publish_config_paths_uses_config_transaction_contract(self) -> None:
        ...
        controller.config._config_values.assert_called_once_with(1, 100.0)
        controller.core.publish_fields.assert_called_once_with(
            "config",
            {"mode": 2},
            100.0,
