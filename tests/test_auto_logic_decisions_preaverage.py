...
from venus_evcharger.auto.logic_types import NO_RELAY_DECISION, RelayDecision, RelayDecisionState
from venus_evcharger.ports.auto import AutoDecisionPort


class PreAverageHarness:
    ...
    def __init__(self) -> None:
        ...
        self.common_decision: RelayDecision = self._NO_DECISION
        self.cutover_decision: RelayDecision = self._NO_DECISION
        self.daytime_window = True
        self.disabled_decision = False
        self.grid_missing_decision = False
        self.grid_recent = True
        self.grid_recovery_decision: RelayDecision = self._NO_DECISION
        ...
        self.resolve_soc_decision: RelayDecision = self._NO_DECISION
        self.scheduled_night = False
        self.scheduled_night_result = True
        self.time_now = 123.0
        context = AutoDecisionContext(
            AutoDecisionPort(self.service),
            lambda _reason: 0,
            self._mode_uses_auto_logic,
        )
        self.workflow = AutoDecisionWorkflow(context, self, self, self, self, self)
    ...
    def handle_common_runtime_gates(
        self,
        relay_on: bool,
        now: float,
        cached_inputs: bool,
    ...
    def scheduled_night_decision(self, relay_on: bool, now: float, cached_inputs: bool) -> bool:
        ...
        return self.scheduled_night_result
...
class TestPreAverageDecisions(unittest.TestCase):
    def test_pre_average_gate_result_distinguishes_pending_from_resolved(self) -> None:
        ...
        self.assertIsNone(harness.workflow._pre_average_gate_result(NO_RELAY_DECISION))

        false_state = RelayDecisionState.resolved(False)
        self.assertEqual(harness.workflow._pre_average_gate_result(false_state), (false_state, None))
        true_state = RelayDecisionState.resolved(True)
        self.assertEqual(harness.workflow._pre_average_gate_result(true_state), (true_state, None))

    def test_mode_decision_orders_non_auto_disabled_and_cutover_gates(self) -> None:
        harness = PreAverageHarness()
        harness.mode_uses_auto = False

        non_auto = harness.workflow._pre_average_mode_decision(relay_on=False, cached_inputs=True)
        ...
        disabled = harness.workflow._pre_average_mode_decision(relay_on=True, cached_inputs=True)
        ...
        harness = PreAverageHarness()
        pending = harness.workflow._pre_average_mode_decision(relay_on=True, cached_inputs=False)
        ...
        cutover = harness.workflow._pre_average_mode_decision(relay_on=True, cached_inputs=False)
        ...
        missing_enable = harness.workflow._pre_average_mode_decision(relay_on=False, cached_inputs=True)
    ...
    def test_input_gate_prioritizes_scheduled_night_grid_missing_and_recovery(self) -> None:
        ...
        self.assertIs(harness.workflow._pre_average_input_gate_decision(False, -100.0, 10.0, True), NO_RELAY_DECISION)
        self.assertEqual(harness.calls, [("scheduled_night_active", 10.0)])

        harness = PreAverageHarness()
        harness.grid_recent = False
        grid_missing = harness.workflow._pre_average_input_gate_decision(True, None, 11.0, True)
        ...
        harness = PreAverageHarness()
        recovery_pending = harness.workflow._pre_average_input_gate_decision(False, -50.0, 12.0, False)
        ...
        recovery_resolved = harness.workflow._pre_average_input_gate_decision(False, -50.0, 12.0, False)
    ...
    def test_battery_soc_decision_preserves_resolved_soc_and_terminal_decision(self) -> None:
        ...
        battery_soc, early = harness.workflow._pre_average_battery_soc_result(70, True, 20.0, False)
        ...
        battery_soc, early = harness.workflow._pre_average_battery_soc_result(None, False, 21.0, True)
    ...
    def test_missing_input_result_requires_pv_and_grid_before_averaging(self) -> None:
        ...
        decision, battery_soc = harness.workflow._pre_average_missing_input_result(
            relay_on=True,
            pv_power=1200.0,
            battery_soc=66.0,
            grid_power=-800.0,
    ...
    def test_pre_average_decision_stops_at_each_terminal_gate(self) -> None:
        ...
        decision, battery_soc = harness.workflow._pre_average_decision(False, 1000.0, 70.0, -500.0, 40.0, False)
        ...
        decision, battery_soc = harness.workflow._pre_average_decision(False, 1000.0, None, -500.0, 42.0, False)
        ...
        decision, battery_soc = harness.workflow._pre_average_decision(True, None, 69.0, -333.0, 43.0, True)
    ...
    def test_post_average_decision_and_average_metrics_report_exact_state(self) -> None:
        ...
        decision, daytime = harness.workflow._post_average_decision(True, 10.0, -20.0, 75.0, 50.0, True)
        ...
        decision, daytime = harness.workflow._post_average_decision(False, 10.0, -20.0, 75.0, 51.0, False)
        ...
        harness = PreAverageHarness()
        self.assertEqual(harness.workflow._averaged_auto_metrics(60.0, 1000.0, -300.0, 80.0, True, False), (500.0, -250.0))
        ...
        self.assertIsNone(harness.workflow._averaged_auto_metrics(61.0, 1000.0, -300.0, 80.0, False, True))
        self.assertEqual(harness.health_calls, [("averaging", True, False)])

        harness = PreAverageHarness()
        harness.average_result = (500.0, None)

        self.assertIsNone(harness.workflow._averaged_auto_metrics(62.0, 1000.0, -300.0, 80.0, True, False))
    ...
    def test_decision_from_averages_uses_terminal_common_gate_or_relay_branch(self) -> None:
        ...
        self.assertTrue(harness.workflow._decision_from_averages(False, 150.0, -50.0, 70.0, 90.0, False))
        ...
        setattr(harness.workflow, "_post_average_decision", post_average_recorder)
        self.assertTrue(harness.workflow._decision_from_averages(True, 153.0, -53.0, 73.0, 93.0, True))
        ...
        harness = PreAverageHarness()
        self.assertTrue(harness.workflow._decision_from_averages(True, 151.0, -51.0, 71.0, 91.0, True))
        self.assertEqual(
            harness.calls,
            [
        ...
        self.assertFalse(harness.workflow._decision_from_averages(False, 152.0, -52.0, 72.0, 92.0, False))
        self.assertEqual(
            harness.calls,
            [
    ...
    def test_auto_decide_relay_forwards_cached_inputs_time_and_preaverage_result(self) -> None:
        ...
        self.assertTrue(harness.workflow.auto_decide_relay(relay_on=False, pv_power=1100.0, battery_soc=77.0, grid_power=-600.0))
        ...
        self.assertFalse(harness.workflow.auto_decide_relay(relay_on=True, pv_power=900.0, battery_soc=55.0, grid_power=-200.0))
        ...
        setattr(harness.workflow, "_pre_average_decision", pre_average_recorder)
        setattr(harness.workflow, "_auto_decision_after_pre_average", after_pre_average_recorder)
        harness.service._auto_cached_inputs_used = True

        self.assertTrue(harness.workflow.auto_decide_relay(relay_on=False, pv_power=901.0, battery_soc=56.0, grid_power=-201.0))
        ...
        self.assertFalse(harness.workflow.auto_decide_relay(relay_on=True, pv_power=902.0, battery_soc=57.0, grid_power=-202.0))
    ...
    def test_required_average_inputs_rejects_missing_values(self) -> None:
        self.assertEqual(AutoDecisionWorkflow._required_average_inputs(1.0, 2.0, 3.0), (1.0, 2.0, 3.0))

        with self.assertRaises(AssertionError):
            AutoDecisionWorkflow._required_average_inputs(None, 2.0, 3.0)
        with self.assertRaises(AssertionError):
            AutoDecisionWorkflow._required_average_inputs(1.0, None, 3.0)
        with self.assertRaises(AssertionError):
            AutoDecisionWorkflow._required_average_inputs(1.0, 2.0, None)

    def test_decision_from_available_averages_returns_current_relay_until_ready(self) -> None:
        harness = PreAverageHarness()

        self.assertTrue(harness.workflow._decision_from_available_averages(True, 60.0, 140.0, False, None))
        self.assertFalse(harness.workflow._decision_from_available_averages(False, 60.0, 140.0, False, None))
        self.assertEqual(harness.calls, [])

        harness.common_decision = True
        self.assertTrue(harness.workflow._decision_from_available_averages(False, 61.0, 141.0, True, (10.0, -5.0)))
        ...
        setattr(harness.workflow, "_decision_from_averages", decision_from_averages_recorder)

        self.assertTrue(harness.workflow._decision_from_available_averages(False, 62.0, 142.0, True, (11.0, -6.0)))
