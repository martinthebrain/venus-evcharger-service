...
class ApplyContracts(unittest.TestCase):
    ...
    def test_initialize_metrics_and_enabled_use_exact_service_contract(self) -> None:
        ...
        self.subject._victron_ess_balance_support_mode = MagicMock(return_value="supported")
        self.subject._victron_ess_balance_activation_mode = MagicMock(return_value="export")
        ...
        self.subject._victron_ess_balance_support_mode.assert_called_once_with(svc)
        self.subject._victron_ess_balance_activation_mode.assert_called_once_with(svc)
    ...
    def test_state_helper_wrappers_delegate_without_changing_arguments(self) -> None:
        svc = _service()
        with patch("venus_evcharger.update.victron_ess_balance_apply._reset_victron_ess_balance_pid_state") as reset:
            self.subject._reset_victron_ess_balance_pid(svc)
            reset.assert_called_once_with(svc)
        with patch(
            "venus_evcharger.update.victron_ess_balance_apply._reset_victron_ess_balance_pid_integral_state"
        ) as reset_integral:
            self.subject._reset_victron_ess_balance_pid_integral(svc)
            self.subject._reset_victron_ess_balance_pid_integral(svc, True)
            self.assertEqual(reset_integral.call_args_list, [call(svc, False), call(svc, True)])
    ...
    def test_disable_merges_before_reset(self) -> None:
        ...
        self.subject._reset_victron_ess_balance_pid = MagicMock(side_effect=lambda *_: events.append("reset"))
        self.subject._disable_victron_ess_balance(svc, metrics)
        self.assertEqual(events, ["merge", "reset"])
        self.subject._merge_victron_ess_balance_metrics.assert_called_once_with(svc, metrics)

    def test_cluster_state_short_circuits_and_enforces_two_sources(self) -> None:
        svc = _service()
        self.subject._normalized_mapping = MagicMock(return_value={"battery_discharge_balance_eligible_source_count": 1})
        self.assertEqual(self.subject._victron_ess_balance_cluster_state(svc, False), ({}, "auto-mode-inactive"))
        self.subject._normalized_mapping.assert_not_called()
        self.assertEqual(
        ...
        self.subject._normalized_mapping.assert_called_once_with(svc._last_energy_cluster)
        self.subject._normalized_mapping.return_value = {}
        self.assertEqual(
            self.subject._victron_ess_balance_cluster_state(svc, True),
            ({}, "insufficient-eligible-sources"),
        )
        self.subject._normalized_mapping.reset_mock()
        self.subject._victron_ess_balance_cluster_state(SimpleNamespace(), True)
        self.subject._normalized_mapping.assert_called_once_with(None)
        cluster = {"battery_discharge_balance_eligible_source_count": 2}
        self.subject._normalized_mapping.return_value = cluster
    ...
    def test_source_state_covers_resolution_health_value_support_and_success(self) -> None:
        svc = _service()
        metrics: dict[str, Any] = {}
        self.subject._victron_ess_balance_source = MagicMock(return_value=(None, "missing"))
        self.assertEqual(self.subject._victron_ess_balance_source_state({}, svc, metrics), (None, None, "missing"))
        self.subject._victron_ess_balance_current_topology_key = MagicMock(return_value="topology")
        self.subject._victron_ess_balance_source_support_allowed = MagicMock(return_value=True)
        source = {"source_id": " victron ", "online": False, "discharge_balance_error_w": -12.0}
        self.subject._victron_ess_balance_source.return_value = (source, "")
        self.assertEqual(
        ...
        self.subject._victron_ess_balance_source.assert_called_with({}, svc)
        self.subject._victron_ess_balance_current_topology_key.assert_called_with(svc, "victron")
        source["online"] = True
        ...
        self.subject._victron_ess_balance_source_support_allowed.return_value = False
        self.assertEqual(
            self.subject._victron_ess_balance_source_state({}, svc, metrics),
            (None, None, "victron-source-support-blocked"),
        )
        self.subject._victron_ess_balance_source_support_allowed.return_value = True
    ...
    def test_learning_state_executes_profile_contract_in_order(self) -> None:
        ...
        events: list[str] = []
        self.subject._victron_ess_balance_learning_profile = MagicMock(return_value=profile)
        self.subject._set_victron_ess_balance_active_profile = MagicMock(side_effect=lambda *_: events.append("set"))
        self.subject._ensure_victron_ess_balance_learning_profile_state = MagicMock(
            side_effect=lambda *_: events.append("ensure")
        )
        self.subject._merge_victron_ess_balance_learning_profile_metrics = MagicMock(
            side_effect=lambda *_: events.append("merge")
        )
        self.subject._victron_ess_balance_refresh_stable_tuning = MagicMock(
            side_effect=lambda *_: events.append("refresh")
        )
        self.subject._victron_ess_balance_note_action_direction = MagicMock(
            side_effect=lambda *_: events.append("direction") or 3
        )
        self.subject._populate_victron_ess_balance_runtime_safety_metrics = MagicMock(
        ...
        self.subject._victron_ess_balance_learning_profile.assert_called_once_with(
            svc, {"c": 1}, {"s": 1}, -4.0
        )
        self.subject._ensure_victron_ess_balance_learning_profile_state.assert_called_once_with(svc, "profile")
        self.subject._merge_victron_ess_balance_learning_profile_metrics.assert_called_once_with(
            svc, metrics, "profile"
        )
        self.subject._victron_ess_balance_refresh_stable_tuning.assert_called_once_with(svc, metrics, 5.0)
        self.subject._victron_ess_balance_note_action_direction.assert_called_once_with(svc, "export", 5.0)
    ...
    def test_tracking_profile_blocks_safety_then_activation_and_returns_key(self) -> None:
        ...
        self.subject._victron_ess_balance_activation_allowed = MagicMock()
        ...
        self.subject._victron_ess_balance_activation_allowed.assert_not_called()
        self.subject._victron_ess_balance_safety_block_reason.return_value = ""
        self.subject._victron_ess_balance_activation_allowed.return_value = False
        self.assertEqual(
            self.subject._prepare_victron_ess_balance_tracking_profile(*args),
            (None, "activation-mode-blocked"),
        )
        self.subject._victron_ess_balance_activation_allowed.assert_called_with(profile, svc)
        self.subject._victron_ess_balance_activation_allowed.return_value = True
    ...
    def test_safety_blocker_short_circuits_and_clears_stale_safe_state(self) -> None:
        svc = _service()
        metrics: dict[str, Any] = {}
        self.subject._victron_ess_balance_overshoot_cooldown_active = MagicMock(return_value=True)
        self.subject._victron_ess_balance_oscillation_lockout_active = MagicMock(return_value=True)
        self.subject._maybe_restore_victron_ess_balance_stable_tuning = MagicMock()
        self.assertEqual(self.subject._victron_ess_balance_safety_block_reason(svc, 8.0, metrics), "overshoot-cooldown-active")
        self.subject._maybe_restore_victron_ess_balance_stable_tuning.assert_called_once_with(
            svc, metrics, "overshoot_cooldown"
        )
        self.subject._victron_ess_balance_oscillation_lockout_active.assert_not_called()
        self.subject._victron_ess_balance_overshoot_cooldown_active.return_value = False
        self.assertEqual(self.subject._victron_ess_balance_safety_block_reason(svc, 9.0, metrics), "oscillation-lockout-active")
        self.subject._maybe_restore_victron_ess_balance_stable_tuning.assert_called_with(
            svc, metrics, "oscillation_lockout"
        )
        self.subject._victron_ess_balance_oscillation_lockout_active.return_value = False
        self.assertEqual(self.subject._victron_ess_balance_safety_block_reason(svc, 10.0, metrics), "")
        self.subject._victron_ess_balance_oscillation_lockout_active.assert_called_with(svc, 10.0)
    ...
    def test_setpoint_and_telemetry_delegation_publish_exact_values(self) -> None:
        ...
        metrics: dict[str, Any] = {}
        self.subject._victron_ess_balance_pid_output = MagicMock(return_value=12.5)
        self.assertEqual(self.subject._victron_ess_balance_tracking_setpoint(svc, 7.0, -20.0, metrics), 52.5)
        self.subject._victron_ess_balance_pid_output.assert_called_once_with(svc, -20.0, 7.0)
        self.assertEqual(
            metrics,
            {
        ...
        self.subject._update_victron_ess_balance_telemetry = MagicMock()
        self.subject._victron_ess_balance_update_tracking_telemetry(svc, 8.0, {"c": 1}, -3.0, "p", metrics)
        self.subject._update_victron_ess_balance_telemetry.assert_called_once_with(
    ...
    def test_write_outcome_and_tracking_write_state_cover_all_branches(self) -> None:
        ...
        self.subject._record_victron_ess_balance_command = MagicMock()
        ...
        self.subject._victron_ess_balance_write_setpoint.reset_mock()
        sparse = SimpleNamespace()
        self.subject._victron_ess_balance_apply_write_outcome(sparse, 10.0, 70.0, -6.0, "p", {})
        self.subject._victron_ess_balance_write_setpoint.assert_called_once_with(sparse, "", "", 70.0)
        ...
        self.subject._victron_ess_balance_last_setpoint = MagicMock(return_value=65.0)
        metrics.clear()
        self.subject._victron_ess_balance_tracking_write_state(svc, 12.0, 72.0, -8.0, "p", metrics)
        self.subject._victron_ess_balance_should_write.assert_called_with(svc, 12.0, 72.0)
