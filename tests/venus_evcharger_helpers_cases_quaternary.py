...
class TestShellyWallboxHelpersQuaternary(ShellyWallboxHelpersTestBase):
    def test_auto_mode_can_stop_running_charge_when_pv_missing_but_soc_is_too_low(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=200.0):
            self.assertTrue(service.auto.decide_relay(True, None, 10.0, 500.0))
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=231.0):
            self.assertFalse(service.auto.decide_relay(True, None, 10.0, 500.0))
    ...
    def test_auto_mode_can_stop_running_charge_when_pv_missing_but_grid_import_is_high(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=200.0):
            self.assertTrue(service.auto.decide_relay(True, None, 40.0, 500.0))
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=231.0):
            self.assertFalse(service.auto.decide_relay(True, None, 40.0, 500.0))
    ...
    def test_update_virtual_state_keeps_enable_separate_from_relay_state(self):
        ...
        service.controllers.runtime.update.components.state.update_virtual_state(6, 1.23, False)
    ...
    def test_update_virtual_state_resets_session_when_relay_stays_on_but_charging_stops(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=130.0):
            service.controllers.runtime.update.components.state.update_virtual_state(6, 6.25, True)
    ...
    def test_update_virtual_state_starts_new_session_after_manual_unplug_and_replug(self):
        ...
        service.virtual_startstop = 1
        service.controllers.runtime.publisher.publish_field = MagicMock(
        ...
        service.controllers.state.save_runtime_state = MagicMock()
        service.controllers.state.save_runtime_overrides = MagicMock()
        service.controllers.state.state_summary = MagicMock(return_value="state")
        ...
        service.time_now = MagicMock(return_value=100.0)
        service.controllers.runtime.update.components.state.update_virtual_state(2, 10.0, True)
        self.assertEqual(service._dbusservice["/Session/Energy"], 0.0)
        ...
        service.time_now.return_value = 130.0
        service.controllers.runtime.update.components.state.update_virtual_state(6, 10.5, True)
        self.assertEqual(service._dbusservice["/Session/Energy"], 0.0)
        self.assertIsNone(service.charging_started_at)
        ...
        service.controllers.runtime.shelly.publish_local_pm_status.assert_not_called()
        self.assertFalse(service._ignore_min_offtime_once)

        service.time_now.return_value = 160.0
        service.controllers.runtime.update.components.state.update_virtual_state(2, 11.0, True)
        self.assertEqual(service._dbusservice["/Session/Energy"], 0.0)
        self.assertEqual(service.charging_started_at, 160.0)
        self.assertEqual(service.energy_at_start, 11.0)
        self.assertEqual(service.virtual_mode, 2)

        service.time_now.return_value = 190.0
        service.controllers.runtime.update.components.state.update_virtual_state(2, 11.4, True)
    ...
    def test_update_virtual_state_keeps_startstop_enabled_in_auto_while_waiting(self):
        ...
        service.controllers.runtime.update.components.state.update_virtual_state(4, 1.23, False)
    ...
    def test_update_virtual_state_keeps_startstop_visible_in_auto_when_relay_is_on(self):
        ...
        service.controllers.runtime.update.components.state.update_virtual_state(2, 1.23, True)
    ...
    def test_auto_mode_starts_after_surplus_delay(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=111.0):
            self.assertTrue(service.auto.decide_relay(False, 2200, 45, -2200))
    ...
    def test_auto_mode_stops_after_import_delay(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=231.0):
            self.assertFalse(service.auto.decide_relay(True, 1500, 45, 400))
    ...
    def test_auto_mode_stops_running_charge_when_grid_value_missing_for_too_long(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=192.0):
            self.assertFalse(service.auto.decide_relay(True, 0.0, 45.0, None))
    ...
    def test_auto_mode_forces_relay_off_when_disabled(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=231.0):
            self.assertFalse(service.auto.decide_relay(True, 2200, 45, -2200))
    ...
    def test_manual_mode_ignores_auto_enable_disable_state(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=231.0):
            self.assertTrue(service.auto.decide_relay(True, 2200, 45, -2200))
    ...
    def test_auto_mode_does_not_start_below_min_soc(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=111.0):
            self.assertFalse(service.auto.decide_relay(False, 2200, 25, -2200))
    ...
    def test_auto_mode_allows_missing_battery_soc_when_enabled(self):
        ...
        with unittest.mock.patch("venus_evcharger_service.time.time", return_value=111.0):
            self.assertTrue(service.auto.decide_relay(False, 2200, None, -2200))
