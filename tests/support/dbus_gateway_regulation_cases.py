...
class GatewayRegulationCases(GatewayAdapterContractCase):
    ...
    def test_slo_regulation_adjusts_burst_reads_and_discovery(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(adapter.write_scheduler.health_tracker.dynamic_local_publish_burst_limit, 1)
            ...
            selective.health_role.apply_slo_regulation()
    ...
    def test_slo_regulation_forwards_exact_backpressure_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            adapter.health_role.cache_freshness_snapshot.assert_called_once_with(1000.0)
            backpressure_snapshot.assert_called_once_with(
                circuit_state="ok",
                queue_health={
                    "pending_command_count": 1,
                    "physical_command_count": 1,
                    "oldest_command_age_s": 20.0,
                    "oldest_slo_command_age_s": 20.0,
            ...
            self.assertEqual(
                ...
                [unittest.mock.call(now=2000.0)],
            )
            adapter.commands.load_pending.assert_called_once_with()
    ...
    def test_slo_regulation_defaults_missing_pressure_states_to_ok(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            install_mock(adapter.health_role, "suspend_advisory_work", MagicMock())
            install_mock(adapter.circuit, "state", MagicMock(return_value="ok"))
            install_mock(adapter.resource_monitor, "snapshot", MagicMock(return_value={}))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            install_mock(
                ...
                MagicMock(
                    return_value={
                        "max_tick_gap_ms_60s": (
                            adapter.health_role.slo_thresholds().mainloop_gap_max_ms + 1.0
                        )
                    }
                ),
            )
            adapter.health_role.apply_slo_regulation()
            self.assertEqual(adapter.write_scheduler.health_tracker.dynamic_local_publish_burst_limit, 10)

            quiet_adapter = DbusAdapter(str(config_path), paths=gateway_paths(str(Path(temp_dir) / "run-quiet")))
            quiet_adapter.discovery.next_scan_monotonic = 0.0
            ...
            install_mock(
                ...
                "pending_snapshot",
                MagicMock(
                    return_value=PendingCommandSnapshot(
                        tuple(advisory),
                        tuple(advisory),
                    )
                ),
            ...
            self.assertEqual(quiet_adapter.discovery.next_scan_monotonic, 160.0)
            ...
            self.assertEqual(
                quiet_adapter.write_scheduler.remove_pending.call_args_list,
                [
                    unittest.mock.call(
                        "introspection.json",
                        {"queue_class": "introspection"},
                    ),
                    unittest.mock.call(
                        "discovery.json",
                        {"queue_class": "discovery"},
                    ),
                    unittest.mock.call(
                        "diagnostic.json",
