...
def _command(target: str, value: object = 1) -> ControlCommand:
    return ControlCommand(name="set_mode", target=target, value=value)
...
class RuntimeAsyncMainloopControlContractTests(unittest.TestCase):
    ...
    def test_async_enqueue_default_limit_keeps_latest_32_paths(self) -> None:
        ...
        with patch("venus_evcharger.runtime.async_mainloop_control.time.time", return_value=100.0):
            for index in range(33):
                self.assertTrue(controller.enqueue_control_command(_command(f"target_{index}", index)))

        self.assertEqual(len(service._control_command_pending), 32)
        self.assertNotIn("target_0", service._control_command_pending)
        self.assertNotIn("target_0", service._desired_control_values)
        self.assertIn("target_32", service._control_command_pending)
    ...
    def test_async_enqueue_tolerates_missing_desired_value_for_dropped_path(self) -> None:
        ...
        old = _command("old")
        service._control_command_pending[old.target] = (1, 90.0, old)
    ...
    def test_drain_uses_queue_order_and_records_exact_lag_and_duration(self) -> None:
        ...
        service._control_command_pending = OrderedDict(
            ((earlier.target, (1, 103.0, earlier)), (later.target, (2, 104.0, later)))
        )
        service.handle_control_command = MagicMock()
    ...
    def test_drain_clamps_negative_lag_and_logs_exact_failure_and_budget(self) -> None:
        service, controller = _controller()
        command = _command("mode")
        service._control_command_pending[command.target] = (1, 110.0, command)
        ...
        exception.assert_called_once_with("Async control command failed target=%s", "mode")
    ...
    def test_drain_budget_is_strict_and_defaults_to_two_seconds(self) -> None:
        ...
        service._control_command_pending[first.target] = (1, 0.0, first)
        ...
        service._control_command_pending[second.target] = (2, 0.0, second)
        ...
        service._control_command_pending[third.target] = (3, 0.0, third)
