...
class DbusPublishLearnedContractTests(unittest.TestCase):
    ...
    def test_display_uses_learned_set_current_requires_no_native_set_current_backend_and_enabled_flag(self) -> None:
        self.assertTrue(_learned(_service())._display_uses_learned_set_current())
        self.assertTrue(
            _learned(_service(_charger_backend=SimpleNamespace(read_only=True)))._display_uses_learned_set_current()
        )
        self.assertFalse(
            _learned(_service(_charger_backend=SimpleNamespace(set_current=MagicMock())))._display_uses_learned_set_current()
        )
        self.assertFalse(_learned(_service(display_learned_set_current=0))._display_uses_learned_set_current())
        self.assertFalse(_learned(_service(display_learned_set_current=False))._display_uses_learned_set_current())

        default_service = _service()
        delattr(default_service, "display_learned_set_current")
        self.assertTrue(_learned(default_service)._display_uses_learned_set_current())
        self.assertTrue(_learned(_service(display_learned_set_current="bad"))._display_uses_learned_set_current())
        self.assertTrue(_learned(_service(display_learned_set_current=2))._display_uses_learned_set_current())
    ...
    def test_charger_state_freshness_and_readbacks_require_backend_timestamp_and_positive_current(self) -> None:
        ...
        self.assertEqual(
            _learned(_service(_dbus_live_publish_interval_seconds=0.0, auto_shelly_soft_fail_seconds=0.0))
            ._charger_state_max_age_seconds(),
            2.0,
        )
        self.assertEqual(
            _learned(_service(_dbus_live_publish_interval_seconds=None, auto_shelly_soft_fail_seconds=None))
            ._charger_state_max_age_seconds(),
            2.0,
        )
        missing_interval = _service(auto_shelly_soft_fail_seconds=10.0)
        delattr(missing_interval, "_dbus_live_publish_interval_seconds")
        self.assertEqual(_learned(missing_interval)._charger_state_max_age_seconds(), 2.0)
        ...
        self.assertEqual(_learned(missing_soft_fail)._charger_state_max_age_seconds(), 2.0)
        ...
        self.assertTrue(harness.charger_enabled_readback(100.0))
        self.assertEqual(harness._charger_current_readback(100.0), 12.5)

        stale = _learned(_service(_charger_backend=None, _last_charger_state_at=100.0))
        self.assertFalse(stale._charger_state_fresh(100.0))
        self.assertIsNone(stale.charger_enabled_readback(100.0))
        missing_state_at = _learned(_service(_charger_backend=object()))
        ...
        self.assertIsNone(_learned(missing_enabled).charger_enabled_readback(100.0))
        self.assertIsNone(_learned(_service(_charger_backend=object(), _last_charger_state_at=100.0))._charger_current_readback(100.0))
        self.assertIsNone(
            _learned(
    ...
    def test_charger_text_estimate_transport_and_retry_diagnostics_are_normalized_and_fresh(self) -> None:
        ...
        harness = _learned(service)

        self.assertEqual(harness.charger_text_observed("_last_charger_text"), "ready")
        self.assertEqual(harness.charger_text_observed("_missing_text"), "")
        self.assertEqual(harness.charger_estimate_active(), 1)
        self.assertEqual(harness.charger_estimate_source(), "meterless")
        self.assertEqual(harness.charger_transport_active(100.0), 1)
        self.assertEqual(harness.charger_transport_reason(100.0), "timeout")
        self.assertEqual(harness.charger_transport_source(100.0), "charger")
        self.assertEqual(harness.charger_transport_detail(100.0), "slow reply")
        self.assertEqual(harness.charger_retry_active(100.0), 1)
        self.assertEqual(harness.charger_retry_reason(100.0), "offline")
        self.assertEqual(harness.charger_retry_source(100.0), "relay")

        self.assertEqual(harness.charger_transport_active(200.0), 0)
        self.assertEqual(harness.charger_retry_active(200.0), 0)

    def test_learned_power_expiry_and_allowed_display_state_are_strict(self) -> None:
        self.assertFalse(
            _learned(_service(auto_learn_charge_power_max_age_seconds=0.0))._learned_charge_power_expired_for_display(200.0)
        )
        self.assertTrue(
            _learned(_service(auto_learn_charge_power_max_age_seconds=1.0))._learned_charge_power_expired_for_display(200.0)
        )
        self.assertTrue(
            _learned(_service(learned_charge_power_updated_at=None))._learned_charge_power_expired_for_display(200.0)
        )
        ...
        self.assertTrue(_learned(missing_max_age)._learned_charge_power_expired_for_display(21700.5))

        missing_updated_at = _service()
        delattr(missing_updated_at, "learned_charge_power_updated_at")
        self.assertTrue(_learned(missing_updated_at)._learned_charge_power_expired_for_display(200.0))
        self.assertTrue(
            _learned(
        ...
        self.assertTrue(_learned(_service())._learned_display_current_allowed(101.0))
        self.assertFalse(_learned(_service(learned_charge_power_state="learning"))._learned_display_current_allowed(101.0))
        self.assertFalse(_learned(_service(display_learned_set_current=0))._learned_display_current_allowed(101.0))
        missing_state = _service()
        delattr(missing_state, "learned_charge_power_state")
        self.assertFalse(_learned(missing_state)._learned_display_current_allowed(101.0))

    def test_learned_display_scalar_phase_voltage_rounding_and_clamping_contract(self) -> None:
        harness = _learned(_service())
        ...
        self.assertEqual(_learned(fallback_phase_service)._learned_display_phase(), "3P")
        ...
        self.assertEqual(_learned(default_phase_service)._learned_display_phase(), "L1")
        invalid_phase_service = _service()
        delattr(invalid_phase_service, "learned_charge_power_phase")
        invalid_phase_service.phase = "bad"
        self.assertIsNone(_learned(invalid_phase_service)._learned_display_phase())
        ...
        self.assertIsNone(_learned(missing_power)._raw_learned_display_values())
        missing_voltage = _service()
        delattr(missing_voltage, "learned_charge_power_voltage")
        self.assertIsNone(_learned(missing_voltage)._raw_learned_display_values())
        ...
        self.assertEqual(_learned(default_voltage_mode)._phase_voltage_for_display_current(400.0, "3P"), 400.0)
        self.assertEqual(_learned(_service(voltage_mode=" PHASE "))._phase_voltage_for_display_current(400.0, "3P"), 400.0)
        line_voltage = _learned(_service(voltage_mode="line"))
        ...
        self.assertEqual(_learned(_service(min_current=6.0, max_current=12.0))._clamped_display_current(20.0), 12.0)
        self.assertEqual(_learned(_service(min_current=6.0, max_current=12.0))._clamped_display_current(3.0), 6.0)
        self.assertEqual(_learned(_service(min_current=None, max_current=None))._clamped_display_current(3.0), 3.0)
        missing_limits = _service()
        delattr(missing_limits, "min_current")
        delattr(missing_limits, "max_current")
        self.assertEqual(_learned(missing_limits)._clamped_display_current(3.0), 3.0)
        self.assertEqual(_learned(_service(min_current=None, max_current=0.0))._clamped_display_current(3.0), 3.0)
        self.assertEqual(_learned(_service(min_current=None, max_current=1.0))._clamped_display_current(3.0), 1.0)
        self.assertEqual(_learned(_service(min_current=None, max_current=2.0))._clamped_display_current(3.0), 2.0)
    ...
    def test_display_set_current_prefers_fresh_charger_readback_then_learned_then_virtual(self) -> None:
        ...
        self.assertEqual(native.display_set_current(100.0), 11.0)

        learned = _learned(_service(learned_charge_power_watts=2990.0, learned_charge_power_voltage=230.0))
        self.assertEqual(learned.display_set_current(101.0), 13.0)
        three_phase = _learned(
            _service(
                phase="3P",
        ...
        self.assertEqual(three_phase.display_set_current(101.0), 30.0)

        fallback = _learned(_service(learned_charge_power_state="unknown", virtual_set_current=9.0))
        self.assertEqual(fallback.display_set_current(101.0), 9.0)

    def test_stable_learned_display_inputs_reject_invalid_raw_or_phase_voltage_values(self) -> None:
        self.assertIsNone(_learned(_service(learned_charge_power_watts=None))._stable_learned_display_inputs(101.0))
        self.assertIsNone(_learned(_service(learned_charge_power_phase="bad"))._stable_learned_display_inputs(101.0))
        self.assertIsNone(_learned(_service(learned_charge_power_voltage=0.0))._stable_learned_display_inputs(101.0))
        inputs = _learned(_service(learned_charge_power_phase="3P"))._stable_learned_display_inputs(101.0)
        self.assertIsNotNone(inputs)
        assert inputs is not None
        self.assertEqual(inputs.phase_count, 3.0)
        self.assertEqual(inputs.phase_voltage_v, 230.0)
        line_inputs = _learned(
