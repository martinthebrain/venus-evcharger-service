...
class DbusAdapterWritePublishMutationContracts(GatewayAdapterContractCase):
    ...
    def test_burst_propagates_live_candidate_state_between_actions(self) -> None:
        with self.adapter_scenario() as scenario:
            ...
            with patch.object(write_publish_module.time, "monotonic", return_value=12.5) as monotonic:
            ...
            self.assertTrue(all(candidate.pending_commands is commands for candidate in candidates))

    def test_burst_uses_dynamic_limit_when_no_override_is_supplied(self) -> None:
        with self.adapter_scenario() as scenario:
            scheduler = scenario.adapter.write_scheduler
            scheduler.dynamic_local_publish_burst_limit = 7
            ...
            install_mock(scheduler, "prioritized_commands", MagicMock(return_value=pending))
            process_candidate = install_mock(
                scheduler,
                "_process_local_publish_candidate",
    ...
    def test_candidate_short_circuits_finished_and_nonlocal_work(self) -> None:
        with self.adapter_scenario() as scenario:
            ...
            candidate = write_publish_module._LocalPublishCandidate(2, 2, pending, 10.0)
            local = evcs_publication({"connected": 1})
            remote = gx_relay_refresh_command(0)
            done = install_mock(scheduler, "_local_publish_burst_done", MagicMock(return_value=True))
            budget_available = install_mock(scheduler, "budget_available", MagicMock(return_value=True))
            process_loaded = install_mock(scheduler, "process_loaded_command", MagicMock(return_value="applied"))

            self.assertEqual(scheduler._process_local_publish_candidate("local.json", local, candidate), "break")
            ...
            self.assertEqual(scheduler._process_local_publish_candidate("remote.json", remote, candidate), "skip")
            budget_available.assert_not_called()
            process_loaded.assert_not_called()
    ...
    def test_candidate_forwards_time_path_command_and_pending_identity(self) -> None:
        with self.adapter_scenario() as scenario:
            ...
            pending: CommandFileList = [("publish.json", command)]
            candidate = write_publish_module._LocalPublishCandidate(0, 4, pending, 20.0)
            install_mock(scheduler, "_local_publish_burst_done", MagicMock(return_value=False))
            budget_available = install_mock(scheduler, "budget_available", MagicMock(return_value=False))
            process_loaded = install_mock(scheduler, "process_loaded_command", MagicMock())

            with patch.object(write_publish_module.time, "time", return_value=101.25) as current_time:
                self.assertEqual(
                    scheduler._process_local_publish_candidate("publish.json", command, candidate),
                    "skip",
                )
            ...
            for outcome, expected in (("applied", "processed"), ("dropped", "processed"), ("deferred", "break")):
                with self.subTest(outcome=outcome):
                    ...
                    with patch.object(write_publish_module.time, "time", return_value=102.5):
                        self.assertEqual(
                            scheduler._process_local_publish_candidate("publish.json", command, candidate),
                            expected,
                        )
    ...
    def test_burst_done_has_exact_count_and_time_boundaries(self) -> None:
        with self.adapter_scenario() as scenario:
            scheduler = scenario.adapter.write_scheduler
            scheduler.local_publish_tick_budget_seconds = 0.075
            elapsed = MagicMock(return_value=False)

            with patch.object(write_publish_module, "budget_elapsed", elapsed):
                self.assertTrue(scheduler._local_publish_burst_done(3, 3, 50.0))
                self.assertTrue(scheduler._local_publish_burst_done(0, -1, 50.0))
                elapsed.assert_not_called()

                self.assertFalse(scheduler._local_publish_burst_done(2, 3, 51.0))
                elapsed.assert_called_once_with(51.0, 0.075)

                elapsed.reset_mock(return_value=True)
                elapsed.return_value = True
                self.assertTrue(scheduler._local_publish_burst_done(2, 3, 52.0))
    ...
    def test_next_local_command_preserves_order_and_uses_one_timestamp(self) -> None:
        with self.adapter_scenario() as scenario:
            ...
            prune = install_mock(scheduler, "prune_budget", MagicMock())
            ...
            with patch.object(write_publish_module.time, "time", return_value=700.5) as current_time:
                selected = scheduler.next_local_publish_command()
    ...
    def test_stale_removal_normalizes_key_and_honors_pending_source(self) -> None:
        with self.adapter_scenario() as scenario:
            ...
            supplied: CommandFileList = [("processed.json", processed)]
            load_pending = install_mock(
                scenario.adapter.commands,
                "load_pending",
            ...
            with patch.object(
                ...
                scheduler.drop_stale_coalesced_commands(
                    "processed.json",
                    processed,
            ...
            self.assertEqual(remove.call_args_list, [call("old-a.json"), call("old-b.json")])

            remove.reset_mock()
            with patch.object(write_publish_module, "stale_coalesced_paths", return_value=[]) as stale_paths:
                scheduler.drop_stale_coalesced_commands("processed.json", processed)
            load_pending.assert_called_once_with()
    ...
    def test_missing_coalesce_key_returns_before_loading_or_scanning(self) -> None:
        with self.adapter_scenario() as scenario:
            ...
            with patch.object(write_publish_module, "stale_coalesced_paths") as stale_paths:
                scheduler.drop_stale_coalesced_commands(
                    "processed.json",
