...
class VictronEssRuntimeRestoreContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.service = _service()
        self.controller = VictronEssRuntimeRestorer()
    ...
    def test_learning_profile_fallbacks_and_collection_filtering(self) -> None:
        ...
        self.assertEqual(
            self.controller._normalized_victron_ess_balance_learning_profiles([]),
            {},
        )
    ...
    def test_learning_payload_requires_schema_topology_and_profile_mapping(self) -> None:
        ...
        for patch_values in invalid_patches:
            payload = dict(valid)
    ...
    def test_runtime_state_dispatches_only_mapping_payloads(self) -> None:
        ...
        with (
            ...
            self.controller.restore_runtime_state(
                self.service,
                {
    ...
    def test_adaptive_restore_rejects_invalid_envelopes_and_preserves_invalid_mode(self) -> None:
        topology_key = self._topology_key()
        original_mode = self.service.auto_battery_discharge_balance_victron_bias_activation_mode
        ...
        with patch.object(
            ...
            self.controller._restore_victron_ess_balance_adaptive_tuning_payload(
                self.service,
                {"schema_version": 9, "topology_key": topology_key},
            )
            self.controller._restore_victron_ess_balance_adaptive_tuning_payload(
                self.service,
                {"schema_version": 2, "topology_key": "wrong"},
            )
        restore_pid.assert_not_called()
