...
class VictronEssPidController:
    ...
    def _victron_ess_balance_pid_gain_config(svc: Any) -> dict[str, float]:
        return {
            "kp": float(getattr(svc, "auto_battery_discharge_balance_victron_bias_kp", None) or 0.0),
            "ki": float(getattr(svc, "auto_battery_discharge_balance_victron_bias_ki", None) or 0.0),
            "kd": float(getattr(svc, "auto_battery_discharge_balance_victron_bias_kd", None) or 0.0),
        }
    ...
    def _victron_ess_balance_pid_limit_config(svc: Any) -> dict[str, float]:
        return {
            "deadband_w": max(
                0.0,
                float(getattr(svc, "auto_battery_discharge_balance_victron_bias_deadband_watts", None) or 0.0),
            ),
            "integral_limit_w": max(
                0.0,
                float(getattr(svc, "auto_battery_discharge_balance_victron_bias_integral_limit_watts", None) or 0.0),
            ),
            "max_abs_w": max(
                0.0,
                float(getattr(svc, "auto_battery_discharge_balance_victron_bias_max_abs_watts", None) or 0.0),
            ),
            "ramp_rate_w_per_second": max(
                0.0,
                float(
                    getattr(svc, "auto_battery_discharge_balance_victron_bias_ramp_rate_watts_per_second", None) or 0.0
                ),
            ),
        }
    ...
    def _victron_ess_balance_pid_integral_output(
        ...
        effective_ki = max(0.0, float(ki))
        integral_output_w += effective_ki * effective_error_w * effective_dt
    ...
    def _victron_ess_balance_pid_output(self, svc: Any, error_w: float, now: float) -> float:
        ...
        integral_output_w = float(getattr(svc, "_victron_ess_balance_pid_integral_output_w", None) or 0.0)
        ...
        last_output_w = float(getattr(svc, "_victron_ess_balance_pid_last_output_w", None) or 0.0)
