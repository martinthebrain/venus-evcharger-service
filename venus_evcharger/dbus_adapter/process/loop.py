...
class DbusAdapterLoop(DbusAdapterIntrospection):
    ...
    def tick(self: DbusAdapterLoopContext) -> bool:
    ...
    def priority_read_performed(self: DbusAdapterLoopContext) -> bool:
        return self.reads_need_priority() and self.poll_one_due_read_once()

    def process_standard_operation_once(self: DbusAdapterLoopContext) -> bool:
    ...
    def reads_need_priority(self: DbusAdapterLoopContext) -> bool:
        return self.read_executor.has_pending_aggregate() or self.core_reads_stale()

    def core_reads_stale(self: DbusAdapterLoopContext) -> bool:
        now = time.time()
        for key in ("grid_power_w", "pv_power_w", "battery_soc"):
            if self.core_read_age(key, now) > self.slo_core_read_max_age_seconds:
                return True
        return False

    def core_read_age(self: DbusAdapterLoopContext, key: str, now: float) -> float:
    ...
    def try_read_then_write(self: DbusAdapterLoopContext) -> bool:
        if self.poll_one_due_read_once():
            self._prefer_read_next = self.reads_need_priority()
            return True
        return self.try_scheduled_write(prefer_read_next=True)

    def try_write_then_read(self: DbusAdapterLoopContext) -> bool:
