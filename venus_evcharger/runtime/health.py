...
"""Runtime source health, retry backoff, and stale-cycle recovery."""
...
from venus_evcharger.runtime.contracts import AgeSeconds, RuntimeStateStorePort

WATCHDOG_TRACEBACK_DUMP_ERRORS = (OSError, RuntimeError, TypeError, ValueError)


class RuntimeHealthMonitor:
    """Track source health and recover stale runtime activity."""
    ...
    def _watchdog_base_timestamp(svc: Any) -> float:
        ...
        return RuntimeHealthMonitor._float_attr(getattr(svc, "started_at", None))

    @staticmethod
    def _watchdog_recovery_suppressed(svc: Any, now: float) -> bool:
        """Return whether watchdog recovery is currently rate-limited."""
        recovery_seconds = RuntimeHealthMonitor._float_attr(
    ...
    def watchdog_recover(self, now: float) -> None:
        ...
        svc = self.service
        if not self.is_update_stale(now):
            return
