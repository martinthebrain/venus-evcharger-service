...
class DbusAdapterIntrospectionContext(Protocol):  # pragma: no cover
    ...
    def introspection_request_payload(self) -> dict[str, Any]: ...
    def enqueue_introspection_requests(self, payload: Mapping[str, Any]) -> int: ...
    ...
    def refresh_services_command(self, command: Mapping[str, Any]) -> CommandOutcome: ...
    def introspect_command_if_healthy(self, command: Mapping[str, Any]) -> CommandOutcome: ...
    def introspect_command(self, command: Mapping[str, Any]) -> CommandOutcome: ...
    def timed_introspection_result(self, service: str, path: str, timeout: float) -> tuple[CommandOutcome, Any]: ...
    def read_introspection_xml(self, service: str, path: str, timeout: float) -> Any: ...
    def drop_failed_introspection(self, service: str, path: str, error: BaseException) -> CommandOutcome: ...
    def record_introspection_xml(self, service: str, path: str, xml_data: Any) -> None: ...
...
class DbusAdapterIntrospectionSnapshotContext(Protocol):  # pragma: no cover
    ...
    def introspection_services_snapshot(self, now: float) -> dict[str, Any]: ...
    def introspection_cache_entries(self) -> list[tuple[str, dict[str, Any]]]: ...
    ...
    def introspection_finding(self, entry: Mapping[str, Any], now: float) -> dict[str, Any]: ...
