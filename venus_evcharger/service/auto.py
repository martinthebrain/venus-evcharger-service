...
class DbusAutoLogic(UpdateCycle):
    ...
    def _get_dbus_value(self, service_name: str, object_path: str) -> Any:
        return self._ensure_dbus_input_controller().get_dbus_value(service_name, object_path)

    def _list_dbus_services(self) -> list[str]:
        return require_str_list(self._ensure_dbus_input_controller().list_dbus_services(), "list_dbus_services")

    def _invalidate_auto_pv_services(self) -> None:
        self._ensure_dbus_input_controller().invalidate_auto_pv_services()

    def _invalidate_auto_battery_service(self) -> None:
        self._ensure_dbus_input_controller().invalidate_auto_battery_service()

    def _resolve_auto_pv_services(self) -> list[str]:
        return require_str_list(
            self._ensure_dbus_input_controller().resolve_auto_pv_services(),
            "resolve_auto_pv_services",
        )

    def _get_pv_power(self) -> float | None:
        return require_float_or_none(self._ensure_dbus_input_controller().get_pv_power(), "get_pv_power")

    def _resolve_auto_battery_service(self) -> str | None:
        return require_str_or_none(
            self._ensure_dbus_input_controller().resolve_auto_battery_service(),
            "resolve_auto_battery_service",
        )

    def _get_battery_soc(self) -> float | None:
        return require_float_or_none(self._ensure_dbus_input_controller().get_battery_soc(), "get_battery_soc")

    def _get_grid_power(self) -> float | None:
        return require_float_or_none(self._ensure_dbus_input_controller().get_grid_power(), "get_grid_power")

    def _add_auto_sample(self, now: float, surplus_power: float, grid_power: float) -> None:
        self._ensure_auto_controller().add_auto_sample(now, surplus_power, grid_power)

    def _clear_auto_samples(self) -> None:
        self._ensure_auto_controller().clear_auto_samples()

    def _average_auto_metric(self, index: int) -> float:
        return require_float(self._ensure_auto_controller().average_auto_metric(index), "average_auto_metric")

    def _mark_relay_changed(self, relay_on: bool, now: float | None = None) -> None:
        self._ensure_auto_controller().mark_relay_changed(relay_on, now)

    def _is_within_auto_daytime_window(self, current_dt: Any = None) -> bool:
        return require_bool(
            self._ensure_auto_controller().is_within_auto_daytime_window(current_dt),
            "is_within_auto_daytime_window",
        )

    def _set_health(self, reason: str, cached: bool = False) -> None:
        self._ensure_auto_controller().set_health(reason, cached)
    ...
    def _auto_decide_relay(
        ...
        return require_bool(
            self._ensure_auto_controller().auto_decide_relay(relay_on, pv_power, battery_soc, grid_power),
            "auto_decide_relay",
        )

    def _control_command_from_write(
        self,
        path: str,
        value: Any,
        ...
        return require_instance(
            self._ensure_write_controller().build_control_command(path, value, source=source),
    ...
    def _handle_control_command(self, command: ControlCommand) -> ControlResult:
        result = require_instance(
            self._ensure_write_controller().handle_control_command(command),
    ...
    def _register_paths(self) -> None:
        self._ensure_bootstrap_controller().register_paths()

    def _fetch_device_info_with_fallback(self) -> dict[str, Any]:
        return require_dict(
            self._ensure_bootstrap_controller().fetch_device_info_with_fallback(),
