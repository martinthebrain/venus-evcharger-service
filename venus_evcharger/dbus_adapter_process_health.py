...
class DbusAdapterHealthMixin:
    ...
    def _slo_observed(
        ...
        return {
            "gui_max_age_s": self._max_cached_path_age(GUI_CRITICAL_PUBLISH_PATHS, now),
...
def _health_log_payload(health: Mapping[str, Any]) -> dict[str, Any]:
...
def _health_log_cache_freshness(cache_freshness: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
def _physical_command_count(pending: list[tuple[str, dict[str, Any]]], physical_count: int | None) -> int:
    return len(pending) if physical_count is None else int(physical_count)


def _status_counts(values: Mapping[str, Mapping[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def _cached_entry_age(entry: object, now: float) -> float:
