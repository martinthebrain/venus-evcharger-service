...
class LatencyWindow:
    ...
    def __init__(self, window_seconds: float = 60.0) -> None:  # pragma: no mutate block
        self.window_seconds = max(1.0, float(window_seconds))
        self._latencies: deque[tuple[float, float]] = deque()
        self._timeouts: deque[float] = deque()

    def record_latency(self, latency_ms: float, *, now: float | None = None) -> None:  # pragma: no mutate block
        current = _now() if now is None else float(now)
        self._latencies.append((current, max(0.0, float(latency_ms))))
        self._prune(current)

    def record_timeout(self, *, now: float | None = None) -> None:  # pragma: no mutate block
        current = _now() if now is None else float(now)
        self._timeouts.append(current)
        self._prune(current)

    def _prune(self, now: float) -> None:  # pragma: no mutate block
