...
class MeasuredValue:
    ...
    def __post_init__(self) -> None:
        ...
        if status in {"fresh", "stale"} and self.value is None:
            raise ValueError(f"{status} measurement requires a value")
        if status in {"fresh", "stale"} and observed_at <= 0.0:
            raise ValueError(f"{status} measurement requires a positive observed_at")
    ...
    def from_payload(cls, payload: object) -> MeasuredValue:
        ...
        exact_fields(
            item,
            required={"value", "observed_at", "status", "confidence", "source_ids", "reason_code"},
