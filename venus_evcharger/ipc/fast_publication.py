...
class FastPublicationQueue:
    ...
    def __init__(
        ...
        *,
        ...
        configured_order_capacity = (
            default_order_capacity if order_capacity is None else int(order_capacity)
        )
        ...
        self._orders = PublicationOrderHistory(
    ...
    def _enqueue_candidate(
        ...
        limit_reason = publication_payload_limit_reason(payload)
        if limit_reason:
    ...
    def _claim_failure(
        ...
        reason = _claim_rejection_reason(claim)
        return self._rejected_enqueue(reason) if reason else None
    ...
    def prepare_durable(self, command: CommandMapping) -> CommandPayload | None:
        ...
        key = str(command.get("coalesce_key") or "").strip()
        candidate_orders = self._durable_candidate_orders(key, publication_field_orders(command))
        claim = self._orders.claim_durable(
            key,
            candidate_orders,
            active_fields=self._active_fields(),
        )
        ...
        if not accepted:
            ...
            return None
        self._remove_fast_fields(key, accepted)
        if claim.superseded_fields:
            self._increment("durable_fields_superseded", len(claim.superseded_fields))
        return filter_publication_payload(command, accepted)

    def record_durable_outcome(self, state: str) -> None:
        if state == "applied" and not self._orders.commit_durable():
    ...
    def pop_next(self, *, now: float | None = None) -> FastPublicationWork | None:
        ...
        eligible = [
            key
            for key, work in self._commands.items()
            if work.retry_at <= current
        ]
        if not eligible:
            return None
        key = min(
    ...
    def _durable_candidate_orders(self, key: str, orders: dict[str, int]) -> dict[str, int]:
        queued = self._commands.get(key)
        if queued is None:
            return orders
        fast_orders = publication_field_orders(queued.command)
        return {
            field: order
            for field, order in orders.items()
            if field not in fast_orders or order > fast_orders[field]
        }
    ...
    def _rejection_reason(self, command: CommandMapping) -> str:
        ...
        return publication_payload_limit_reason(command)
    ...
    def _prune_expired(self, now: float) -> None:
        for key, work in tuple(self._commands.items()):
            ...
            if retained is None:
                del self._commands[key]
                self._increment("expired")
            elif len(retained.field_expires_at) != len(work.field_expires_at):
                self._commands[key] = retained
                self._increment(
                    "fields_expired",
                    len(work.field_expires_at) - len(retained.field_expires_at),
                )
    ...
    def _active_fields(self) -> set[PublicationFieldKey]:
        return {
            (key, field)
            for key, work in self._commands.items()
            for field in work.field_expires_at
        }
...
def _claim_rejection_reason(claim: PublicationFieldClaim) -> str:
    if claim.state == "full":
        return "order-history-full"
    if claim.state == "state-error":
