...
_SOCKET_TEST_TARGETS = frozenset(
    {
...
def mutmut_config_for_target(repo: Path, target_path: str) -> Generator[None, None, None]:
...
def mutmut_config_toml(target_path: str) -> str:
    ...
    pytest_args = pytest_add_cli_args_for_target(target_path)
    also_copy = [
    ...
    lines.extend([
        "]",
        f"pytest_add_cli_args = {list(pytest_args)!r}",
...
def pytest_add_cli_args_for_target(target_path: str) -> tuple[str, ...]:
    """Keep socket scenarios enabled only for targets whose behavior depends on them."""
    if target_path in _SOCKET_TEST_TARGETS:
        return ()
    return ("-k", "not socket")
