...
if [[ -x "$PROJECT_VENV_PYRIGHT" ]] && "$PROJECT_VENV_PYRIGHT" --version >/dev/null 2>&1; then
	PYRIGHT_CMD=("$PROJECT_VENV_PYRIGHT")
elif [[ -x "${PYRIGHT_BIN:-}" ]]; then
	PYRIGHT_CMD=("${PYRIGHT_BIN}")
elif command -v pyright >/dev/null 2>&1 && "$(command -v pyright)" --version >/dev/null 2>&1; then
